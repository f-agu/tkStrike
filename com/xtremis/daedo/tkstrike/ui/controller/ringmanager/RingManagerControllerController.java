...
public class RingManagerControllerController extends BaseCommonRingManagerController<NetworkConfigurationEntry> {
	...
	public void doChoseLastMatch() {
		...
		TkStrikeExecutors.executeInThreadPool(new Callable<Void>() {
			...
			public Void call() throws Exception {
				try {
					MatchLogDto lastMatchLog = RingManagerControllerController.this.matchLogService.getLastStarted();
					if(lastMatchLog != null) {
						final MatchConfigurationEntry lastMatchConfig = RingManagerControllerController.this.matchConfigurationService.getEntryById(
								lastMatchLog.getMatchConfigurationId());
							...
							if(lastMatchConfig.isReadyForStart()) {
								Platform.runLater(new Runnable() {
									...
									public void run() {
										...
										RingManagerControllerController.this.getAppStatusWorker().setMatchConfigurationEntry(
												RingManagerControllerController.this.matchConfigurationEntry);
										RingManagerControllerController.this._refreshAllSteps();
									}
								});
							} else {
								RingManagerControllerController.this.showProgressIndicator(false);
								RingManagerControllerController.this.showMonologInfoDialog(RingManagerControllerController.this.getMessage(
										"title.default.error"), RingManagerControllerController.this.getMessage("message.warning.matchIsNotReady"));
							}
						} else {
							RingManagerControllerController.this.showProgressIndicator(false);
						}
	...
	public void doRestoreLastMatch() {
		showProgressIndicator(true);
		try {
			final MatchLogDto lastMatchLog = this.matchLogService.getLastStarted();
			if(lastMatchLog != null)
				if(lastMatchLog.getMatchVictoryCriteria() != null && lastMatchLog
					...
					Optional opResponse = showConfirmDialog(getMessage("message.confirmDialog.title"),
							getMessage("message.confirm.restoreLastMatch", new String[] {lastMatchLog.getMatchNumber(), getDfFullFormat().format(
									new Date(lastMatchLog.getMatchStartTime().longValue()))}));
					if(opResponse != null && opResponse.isPresent() && opResponse.get().equals(ButtonType.OK)) {
						final MatchConfigurationEntry lastMatchConfig = this.matchConfigurationService.getEntryById(lastMatchLog
								.getMatchConfigurationId());
						if(lastMatchConfig != null)
							Platform.runLater(new Runnable() {
								...
								public void run() {
									RingManagerControllerController.this.matchConfigurationEntry = lastMatchConfig;
									RingManagerControllerController.this.updateDifferentialScoreIfNeed();
									RingManagerControllerController.this.getAppStatusWorker().doResetMatchWithMatchLogId(
											RingManagerControllerController.this.matchConfigurationEntry, lastMatchLog.getId());
									RingManagerControllerController.this._refreshAllSteps();
								}
							});
					}
	...
	public void doRestoreDefaultConfig() {
		if(showConfirmDialog(getMessage("title.default.question"),
				getMessage("message.confirmDialog.restoreDefaultConfig1")).get().equals(ButtonType.OK) && showConfirmDialog(getMessage(
	...
	public void openAthletesManagement() {
		openInNewStage(this.athletesManagementController, getMessage("title.window.athletesManagement"), 900, 600);
	}

	public void openPhasesManagement() {
		openInNewStage(this.phasesManagementController, getMessage("title.window.phaseManagement"), 900, 600);
	}
	...
	public void openDifferentialScoreDefinitions() {
		openInNewStage(this.differentialScoreDefinitionManagementController, getMessage("title.window.categories"), 900, 600);
	}
	...
	private void doOpenMatchSelection(MatchConfigurationManagementController.OpenType openType) {
		...
		openInNewStage(this.matchConfigurationManagementController, event -> {
			showProgressIndicator(true);
	...
	private void updateDifferentialScoreIfNeed() {
		if(this.matchConfigurationEntry.getPhase() != null && this.matchConfigurationEntry.getPhase().getId() != null && this.matchConfigurationEntry
			...
			try {
				differentialScoreDefinitionEntry = this.differentialScoreDefinitionService.getEntryByPhaseIdAndSubCategoryId(
						this.matchConfigurationEntry.getPhase().getId(), this.matchConfigurationEntry.getSubCategory().getId());
				if(differentialScoreDefinitionEntry != null) {
					...
					this.matchConfigurationEntry = this.matchConfigurationService.updateByEntry(
							this.matchConfigurationEntry);
				}
			} catch(TkStrikeServiceException e) {
				manageException(e, "", null);
			}
	...
	public void openWizard() {
		openInNewStage(this.ringManagerWizardController, new EventHandler<WindowEvent>() {
			...
			public void handle(WindowEvent windowEvent) {
				if( ! RingManagerControllerController.this.ringManagerWizardController.isWizardCanceled()) {
					...
					final MatchConfigurationEntry selected = RingManagerControllerController.this.ringManagerWizardController
					...
					if(selected != null && StringUtils.isNotBlank(selected.getId())) {
						if(selected.isReadyForStart()) {
							...
							RingManagerControllerController.this.showProgressIndicator(false);
							RingManagerControllerController.this.showMonologConfirmDialog(RingManagerControllerController.this.getMessage(
									"title.default.error"), RingManagerControllerController.this.getMessage("message.warning.matchIsNotReady"));
						}
					} else {
						RingManagerControllerController.this.showProgressIndicator(false);
	...
	public void openFlagsManagement() {
		openInNewStage(this.flagsManagementController, getMessage("title.window.flagsManagement"), 900, 600);
	}
			...
			public void run() {
				...
				try {
					...
					if(externalConfigEntry != null) {
						...
						List<MatchConfigurationDto> matches = RingManagerControllerController.this.wtOvrClientService.findWtOvrMatches(
						...
						if(matches != null) {
							...
							for(MatchConfigurationDto match : matches) {
								if(match != null && match.getInternalId() != null) {
									...
									MatchConfigurationEntry newMatchConfigurationEntry = RingManagerControllerController.this.matchConfigurationService
											.transformByDto(match, true);
									RingManagerControllerController.logger.info("Hem transformat i desat a la base de dades el match "
											+ newMatchConfigurationEntry.getId());
									matchesImported++;
									continue;
								}
								RingManagerControllerController.logger.info("Get a match without internalId ->" + ReflectionToStringBuilder
				...
				if(matchesImported > 0)
					RingManagerControllerController.this.showInfoDialog(RingManagerControllerController.this.getMessage("title.default.info"),
							RingManagerControllerController.this.getMessage("message.info.wtOvrMatchesImported", new String[] {""
									+ matchesImported}));
				RingManagerControllerController.this.showProgressIndicator(false);
			}
		});
	...
	protected void showProgressIndicator(final boolean show) {
		Platform.runLater(new Runnable() {
			...
			public void run() {
				...
				RingManagerControllerController.this.lblPiMessage.setText(RingManagerControllerController.this.getMessage(
						"message.info.workingPleaseWait"));
			}
		});
	}
		...
		if(this.allowVM)
			TkStrikeExecutors.executeInThreadPool(new Runnable() {
				...
				public void run() {
					...
					boolean showError = true;
					try {
						MatchConfigurationDto theMatch = next ? RingManagerControllerController.this.venueManagementClient.getNextMatch()
								: RingManagerControllerController.this.venueManagementClient.getPrevMatch();
						if(theMatch != null) {
							final MatchConfigurationEntry newMatchConfigurationEntry = RingManagerControllerController.this.matchConfigurationService
									.transformByDto(theMatch, false);
							if(newMatchConfigurationEntry != null) {
								if(theMatch.getDifferencialScore() == null || theMatch.getDifferencialScore().equals(Integer.valueOf(0))) {
									newMatchConfigurationEntry.differencialScoreProperty().set(RingManagerControllerController.this
											.getAppStatusWorker().getRulesEntry().getDifferencialScore());
									if(newMatchConfigurationEntry.getPhase() != null && newMatchConfigurationEntry.getPhase().getId() != null
											&& newMatchConfigurationEntry
													.getSubCategory() != null && newMatchConfigurationEntry.getSubCategory().getId() != null) {
										DifferentialScoreDefinitionEntry differentialScoreDefinitionEntry = RingManagerControllerController.this.differentialScoreDefinitionService
												.getEntryByPhaseIdAndSubCategoryId(newMatchConfigurationEntry.getPhase().getId(),
														newMatchConfigurationEntry.getSubCategory().getId());
										if(differentialScoreDefinitionEntry != null)
											newMatchConfigurationEntry.differencialScoreProperty().set(differentialScoreDefinitionEntry.getValue());
									}
								}
								if(newMatchConfigurationEntry.getMaxAllowedGamJeoms() == null || newMatchConfigurationEntry.getMaxAllowedGamJeoms()
										.equals(Integer.valueOf(0)))
									newMatchConfigurationEntry.setMaxAllowedGamJeoms(RingManagerControllerController.this.maxGamJeomsAllowed
											.intValue());
								...
								if(newMatchConfigurationEntry.isReadyForStart()) {
									Platform.runLater(new Runnable() {
										...
										public void run() {
											...
											if(RingManagerControllerController.logger.isDebugEnabled())
												RingManagerControllerController.logger.debug("Has new Match -" + ToStringBuilder.reflectionToString(
														RingManagerControllerController.this.matchConfigurationEntry));
											RingManagerControllerController.this.getAppStatusWorker().setMatchConfigurationEntry(
													RingManagerControllerController.this.matchConfigurationEntry);
											if(RingManagerControllerController.logger.isDebugEnabled())
									...
									RingManagerControllerController.this.showProgressIndicator(false);
									RingManagerControllerController.this.showMonologInfoDialog(RingManagerControllerController.this.getMessage(
											"title.default.error"), RingManagerControllerController.this.getMessage(
													"message.warning.matchIsNotReady"));
								}
							}
						}
					} catch(TkStrikeServiceException e) {
						RingManagerControllerController.this.showProgressIndicator(false);
						showError = false;
						RingManagerControllerController.this.manageException(e, "RequestMatch to VM", e.getMessage());
					} catch(Exception e) {
						e.printStackTrace();
					}
					if(showError) {
						RingManagerControllerController.this.showProgressIndicator(false);
						RingManagerControllerController.this.showErrorDialog(RingManagerControllerController.this.getMessage("title.default.error"),
								RingManagerControllerController.this.getMessage("message.error.venueManagement"));
					}
				}
			});
	}
	...
	void _onWindowShowEvent() {
		...
		try {
			...
			manageException(e, "RingManager getExternalConfig", null);
		}
	...
	protected void _refreshAllSteps() {
		...
		if(this.matchConfigurationEntry != null) {
			if(StringUtils.isNotBlank(this.matchConfigurationEntry.getMatchNumber())) {
				...
				this.lblMatchNumberInfo.setText(this.matchConfigurationEntry.getMatchNumber() + (this.matchConfigurationEntry.isParaTkdMatch()
						? " (PARA TKD)"
						: ""));
			}
			...
			if(this.matchConfigurationEntry.getPhase() != null &&
				...
				this.lblPhaseInfo.setText(getMessage("message.phaseWithDiffScore", new String[] {this.matchConfigurationEntry.getPhase().getName(), ""
						+ this.matchConfigurationEntry.getDifferencialScore()}));
			}
			if(this.matchConfigurationEntry.getCategory() != null &&
				...
				this.lblCategoryInfo.setText(this.matchConfigurationEntry.getCategory().getSubCategory().getName() + " "
						+ this.matchConfigurationEntry
			...
			if(this.matchConfigurationEntry.getBlueAthlete() != null &&
				...
				this.lblBlueInfo.setText(this.matchConfigurationEntry.getBlueAthlete().getScoreboardName() + " - " + ((this.matchConfigurationEntry
						.getBlueAthlete().getFlag() != null) ? (this.matchConfigurationEntry.getBlueAthlete().getFlag().isShowName()
								? this.matchConfigurationEntry.getBlueAthlete().getFlag().getName()
								: this.matchConfigurationEntry.getBlueAthlete().getFlag().getAbbreviation()) : ""));
			...
			if(this.matchConfigurationEntry.getRedAthlete() != null &&
				...
				this.lblRedInfo.setText(this.matchConfigurationEntry.getRedAthlete().getScoreboardName() + " - " + ((this.matchConfigurationEntry
						.getRedAthlete().getFlag() != null) ? (this.matchConfigurationEntry.getRedAthlete().getFlag().isShowName()
								? this.matchConfigurationEntry.getRedAthlete().getFlag().getName()
								: this.matchConfigurationEntry.getRedAthlete().getFlag().getAbbreviation()) : ""));
