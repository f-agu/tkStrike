...
public abstract class CommonRoundFinishedConfirmationController<NE extends INetworkConfigurationEntry<? extends NetworkConfigurationEntity>, MW extends CommonMatchWorker, SBE extends ScoreboardEditorController, MLW extends TkStrikeController>
			...
			public void handle(WindowEvent windowEvent) {
				...
				if(CommonRoundFinishedConfirmationController.this.confirmRoundFinished.get()) {
					if(CommonRoundFinishedConfirmationController.this.bestOf3RoundWithTie.get()) {
						CommonRoundFinishedConfirmationController.this.getMatchWorker().confirmRoundEndsWithWinner(
								(MatchWinner)CommonRoundFinishedConfirmationController.this.tgBestOf3RoundWinner.getSelectedToggle().getUserData());
					} else {
	...
	public void afterPropertiesSet() throws Exception {
		...
		this.tgBestOf3RoundWinner.getToggles().addAll(new Toggle[] {this.tgBestOf3WinnerBlue, this.tgBestOf3WinnerRed});
		this.tgBestOf3WinnerBlue.setUserData(MatchWinner.BLUE);
		this.tgBestOf3WinnerRed.setUserData(MatchWinner.RED);
		this.tgBestOf3RoundWinner.selectToggle(this.tgBestOf3WinnerBlue);
		this.tgBestOf3RoundWinner.selectedToggleProperty().addListener(new ChangeListener<Toggle>() {
			...
			public void changed(ObservableValue<? extends Toggle> observable, Toggle oldValue, Toggle newValue) {
				...
				if(CommonRoundFinishedConfirmationController.this.pnBestOf3SelectWinner.isVisible()) {
					CommonRoundFinishedConfirmationController.this.getAppStatusWorker().bestOf3SuperiorityRoundWinnerProperty().setValue(
							MatchWinner.TIE);
					if(newValue == null) {
						...
						CommonRoundFinishedConfirmationController.this.getAppStatusWorker().bestOf3SuperiorityRoundWinnerProperty().setValue(newValue
								.getUserData());
					}
				}
			}
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
				if( ! newValue.booleanValue() && CommonRoundFinishedConfirmationController.this.open.get()) {
					CommonTkStrikeBaseController.logger.info(
							"roundFinishedOpenProperty changes to false and this form are showing... close it without confirmation!");
