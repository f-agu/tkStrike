...
public abstract class CommonTkStrikeBaseController<NE extends INetworkConfigurationEntry>
		implements Initializable, InitializingBean, TkStrikeController {
	...
	protected static final Image IMAGE_VIDEO = new Image(CommonRoundFinishedConfirmationController.class.getResourceAsStream(
			"/images/ico-video.png"));

	protected static final Image IMAGE_NO_VIDEO = new Image(CommonRoundFinishedConfirmationController.class.getResourceAsStream(
			"/images/ico-novideo.png"));
	...
	protected NE getCurrentNetworkConfiguration() {
		try {
			...
			manageException(e, "TkStrikeBase - getNetworkConfigurationEntry", null);
			return null;
		}
	}
	...
	protected String getMessage(String messageKey, String... params) {
		return this.tkStrikeMessageSource.getMessage(messageKey, params,
	...
	protected void openInNewStage(final TkStrikeController tkStrikeController, EventHandler<WindowEvent> windowCloseEventHandler, String stageTitle,
			int width, int height, boolean resizeable, boolean closeWithExitPress) {
		Scene scene = null;
		...
		scene.getStylesheets().addAll(this.tkStrikeStyleSheetsHelper.getTkStrikeStyleSheets());
			...
			scene.setOnKeyPressed(new EventHandler<KeyEvent>() {
				...
				public void handle(KeyEvent keyEvent) {
					if(keyEvent.getCode().equals(KeyCode.ESCAPE))
						stage.fireEvent(new WindowEvent(CommonTkStrikeBaseController.this.getCurrentStage(), WindowEvent.WINDOW_CLOSE_REQUEST));
				}
			});
	...
	protected void doCloseThisStage() {
		getCurrentStage().fireEvent(new WindowEvent(getCurrentStage(), WindowEvent.WINDOW_CLOSE_REQUEST));
	}
	...
	protected Optional<ButtonType> showAlertWithCheckbox(String title, String message, final String checkBoxText, final Consumer<Boolean> opAction) {
		...
		alert.setDialogPane(new DialogPane() {
			...
			protected Node createDetailsButton() {
				...
				optOut.setOnAction(e -> opAction.accept(Boolean.valueOf(optOut.isSelected())));
				return optOut;
			}
		});
		alert.getDialogPane().setContentText(message);
		alert.getDialogPane().getButtonTypes().addAll(new ButtonType[] {ButtonType.YES, ButtonType.NO});
	...
	protected void manageException(Throwable exception, String source, final String message2Show) {
		...
		Platform.runLater(new Runnable() {
			...
			@Override
			public void run() {
				CommonTkStrikeBaseController.this.showErrorDialog(CommonTkStrikeBaseController.this.getMessage("title.default.error"),
						(message2Show != null) ? message2Show
								: CommonTkStrikeBaseController.this
