...
public abstract class CommonMatchFinalResultController<NE extends INetworkConfigurationEntry, MW extends CommonMatchWorker, SBE extends ScoreboardEditorController, MLW extends TkStrikeController>
		...
		if(this.toggleGroupFinalDecision.getSelectedToggle() != null && this.toggleGroupWinner.getSelectedToggle() != null) {
			if(MatchWinner.TIE.equals(this.toggleGroupWinner.getSelectedToggle().getUserData())) {
				...
				Optional<ButtonType> confirmation = showAlertWithCheckbox(
						...
						getMessage("checkbox.confirm.matchFinishedOnTie"), p -> confValidation.setValue(p.booleanValue()));
				if( ! confirmation.isPresent() || ! confirmation.get().equals(ButtonType.YES) ||
			...
			getMatchWorker().confirmFinalResult((MatchWinner)this.toggleGroupWinner.getSelectedToggle().getUserData(),
					(FinalDecision)this.toggleGroupFinalDecision
	...
	public void openScoreboardEditor() {
		openInNewStage(getScoreboardEditorController(), new EventHandler<WindowEvent>() {
	...
	public void openMatchLog() {
		openInNewStage(getMatchLogViewerController(), getMessage("title.window.matchLog"), 1200, 600);
	}
	...
	public EventHandler<WindowEvent> getOnWindowCloseEventHandler() {
		return new EventHandler<WindowEvent>() {
			...
			public void handle(WindowEvent windowEvent) {
				...
				if( ! CommonMatchFinalResultController.this.callConfirm && CommonMatchFinalResultController.this.closeWithButton
						&& MatchStatusId.MATCH_NEEDS_CONFIRM_FINAL_DECISION

								.equals(CommonMatchFinalResultController.this.getMatchWorker().getCurrentMatchStatus())
						&& CommonMatchFinalResultController.this
								.getMatchWorker().isChangeMatchStatusToTimeoutOnScoreboardChanges()) {
					CommonMatchFinalResultController.this.getMatchWorker().confirmFinalResult(
							(MatchWinner)CommonMatchFinalResultController.this.toggleGroupWinner.getSelectedToggle().getUserData(),
	...
	public void onWindowShowEvent() {
		...
		if(MatchStatusId.MATCH_NEEDS_CONFIRM_FINAL_DECISION.equals(getMatchWorker().getCurrentMatchStatus())) {
			...
			this.tgShowSuperiority.setVisible(((getMatchWorker().isGoldenPointWorking() && getMatchWorker().isGoldenPointTieBreaker())
					|| getMatchWorker().superiorityByRoundsProperty().get()));
			this.tgShowSuperiority.setSelected((getMatchWorker().showGoldenPointTieBreakerOnScoreboard().get() || getMatchWorker()
					.showSuperiorityByRoundsProperty().get()));
		} else {
		...
		this.toggleGroupFinalDecision.getToggles().removeAll(new Toggle[] {this.tbFinalDecisionPTS, this.tbFinalDecisionIRM,
				this.tbFinalDecisionCSC});
		this.pnWinnerByContainer.getChildren().removeAll(new Node[] {this.tbFinalDecisionPTS, this.tbFinalDecisionIRM,
				this.tbFinalDecisionCSC});
		this.pnWinnerByContainer.setVgap(this.defaultWinnerByContainerVGap.intValue());
		if(getMatchWorker().isParaTkdMatch()) {
			this.toggleGroupFinalDecision.getToggles().addAll(new Toggle[] {this.tbFinalDecisionPTS, this.tbFinalDecisionIRM,
					this.tbFinalDecisionCSC});
			this.pnWinnerByContainer.getChildren().addAll(new Node[] {this.tbFinalDecisionPTS, this.tbFinalDecisionIRM,
					this.tbFinalDecisionCSC});
	...
	public final void afterPropertiesSet() throws Exception {
		this.toggleGroupWinner.getToggles().addAll(new Toggle[] {this.tgWinnerBLUE, this.tgWinnerTIE, this.tgWinnerRED});
		...
		this.toggleGroupFinalDecision.getToggles().addAll(new Toggle[] {this.tbFinalDecisionRSC, this.tbFinalDecisionPTF,
				this.tbFinalDecisionPTG, this.tbFinalDecisionGDP, this.tbFinalDecisionSUP, this.tbFinalDecisionWDR, this.tbFinalDecisionDSQ,
				this.tbFinalDecisionDQB, this.tbFinalDecisionPUN});
		if(this.isTkStrikeKTAVersion.booleanValue()) {
			this.defaultWinnerByContainerVGap = Integer.valueOf(5);
			this.toggleGroupFinalDecision.getToggles().addAll(new Toggle[] {this.tbFinalDecisionBKO, this.tbFinalDecisionHKO});
			this.pnWinnerByContainer.getChildren().addAll(new Node[] {this.tbFinalDecisionBKO, this.tbFinalDecisionHKO});
		}
		this.pnWinnerByContainer.setVgap(this.defaultWinnerByContainerVGap.intValue());
				...
				if(newToggle != null) {
					...
					if(finalDecision != null)
						Platform.runLater(new Runnable() {
							...
							@Override
							public void run() {
								CommonMatchFinalResultController.this.lblSelectedFinalDecision.setText(finalDecision.toString() + " - "
										+ CommonMatchFinalResultController.this.getMessage("label.finalDecision." + finalDecision.toString()));
							}
						});
				}
			}
		...
		this.tbFinalDecisionBKO.getStyleClass().addAll(new String[] {"fd-finalDecision-toggle"});
		...
		this.tbFinalDecisionHKO.getStyleClass().addAll(new String[] {"fd-finalDecision-toggle"});
		...
		this.tbFinalDecisionPTS.getStyleClass().addAll(new String[] {"fd-finalDecision-toggle"});
		...
		this.tbFinalDecisionIRM.getStyleClass().addAll(new String[] {"fd-finalDecision-toggle"});
		...
		this.tbFinalDecisionCSC.getStyleClass().addAll(new String[] {"fd-finalDecision-toggle"});
		...
		this.tgShowSuperiority.selectedProperty().addListener(new ChangeListener<Boolean>() {
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
				if(newValue.booleanValue()) {
					CommonMatchFinalResultController.this.tgShowSuperiority.setText(CommonMatchFinalResultController.this.getMessage(
							"toggle.showSuperiority.enabled"));
				} else {
					CommonMatchFinalResultController.this.tgShowSuperiority.setText(CommonMatchFinalResultController.this.getMessage(
							"toggle.showSuperiority.disabled"));
				}
			}
		});
		this.tgShowSuperiority.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				if(CommonMatchFinalResultController.this.getMatchWorker().isGoldenPointWorking() && CommonMatchFinalResultController.this
