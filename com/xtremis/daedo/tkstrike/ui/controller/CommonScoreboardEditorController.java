...
public abstract class CommonScoreboardEditorController<NE extends INetworkConfigurationEntry, MW extends CommonMatchWorker> extends CommonTkStrikeBaseController<NE> implements ScoreboardEditorController {
  ...
  public final EventHandler<WindowEvent> getOnWindowCloseEventHandler() {
    return new EventHandler<WindowEvent>() {
        public void handle(WindowEvent windowEvent) {
          CommonScoreboardEditorController.this._doBeforeOnWindowCloseEventHandler();
          CommonScoreboardEditorController.this.bluePoints.removeListener(CommonScoreboardEditorController.this.bluePenaltiesListener);
          CommonScoreboardEditorController.this.bluePenalties.removeListener(CommonScoreboardEditorController.this.bluePenaltiesListener);
          CommonScoreboardEditorController.this.redPoints.removeListener(CommonScoreboardEditorController.this.redPointsListener);
          CommonScoreboardEditorController.this.redPenalties.removeListener(CommonScoreboardEditorController.this.redPenaltiesListener);
          CommonScoreboardEditorController.this.bluePoints.set(0);
          CommonScoreboardEditorController.this.blueGeneralPoints.set(0);
          CommonScoreboardEditorController.this.blueGoldenPointNearMissHits.set(0);
          CommonScoreboardEditorController.this.redPoints.set(0);
          CommonScoreboardEditorController.this.redGeneralPoints.set(0);
          CommonScoreboardEditorController.this.redGoldenPointNearMissHits.set(0);
          CommonScoreboardEditorController.this._doAfterOnWindowCloseEventHandler();
        }
      };
  }
  ...
  public final void onWindowShowEvent() {
    ...
    this.bluePenalties.set(getMatchWorker().isGoldenPointWorking() ? getMatchWorker().getBlueGoldenPointPenalties() : getMatchWorker().getBluePenalties());
    this.redPenalties.set(getMatchWorker().isGoldenPointWorking() ? getMatchWorker().getRedGoldenPointPenalties() : getMatchWorker().getRedPenalties());
    ...
    Platform.runLater(new Runnable() {
          public void run() {
            CommonScoreboardEditorController.this.btConfirmNewScore.requestFocus();
          }
        });
  }
        ...
        switch (scoreboardEditAction.getAction()) {
          case BODY_POINT:
            if (scoreboardEditAction.isBlue()) {
              getMatchWorker().addBlueBodyPoint(scoreboardEditAction.getValue(), ActionSource.SCOREBOARD_EDITOR, newTime, matchRoundTimestamp, 0);
              break;
            } 
            getMatchWorker().addRedBodyPoint(scoreboardEditAction.getValue(), ActionSource.SCOREBOARD_EDITOR, newTime, matchRoundTimestamp, 0);
            break;
          case BODY_TECH_POINT:
            if (scoreboardEditAction.isBlue()) {
              getMatchWorker().addBlueBodyTechPoint(scoreboardEditAction.getValue(), ActionSource.SCOREBOARD_EDITOR, newTime, matchRoundTimestamp);
              break;
            } 
            getMatchWorker().addRedBodyTechPoint(scoreboardEditAction.getValue(), ActionSource.SCOREBOARD_EDITOR, newTime, matchRoundTimestamp);
            break;
          case HEAD_POINT:
            if (scoreboardEditAction.isBlue()) {
              getMatchWorker().addBlueHeadPoint(scoreboardEditAction.getValue(), ActionSource.SCOREBOARD_EDITOR, newTime, matchRoundTimestamp, 0);
              break;
            } 
            getMatchWorker().addRedHeadPoint(scoreboardEditAction.getValue(), ActionSource.SCOREBOARD_EDITOR, newTime, matchRoundTimestamp, 0);
            break;
          case HEAD_TECH_POINT:
            if (scoreboardEditAction.isBlue()) {
              getMatchWorker().addBlueHeadTechPoint(scoreboardEditAction.getValue(), ActionSource.SCOREBOARD_EDITOR, newTime, matchRoundTimestamp);
              break;
            } 
            getMatchWorker().addRedHeadTechPoint(scoreboardEditAction.getValue(), ActionSource.SCOREBOARD_EDITOR, newTime, matchRoundTimestamp);
            break;
          case PUNCH_POINT:
            if (scoreboardEditAction.isBlue()) {
              getMatchWorker().addBluePunchPoint(scoreboardEditAction.getValue(), ActionSource.SCOREBOARD_EDITOR, newTime, matchRoundTimestamp);
              break;
            } 
            getMatchWorker().addRedPunchPoint(scoreboardEditAction.getValue(), ActionSource.SCOREBOARD_EDITOR, newTime, matchRoundTimestamp);
            break;
          case GAME_JEON:
            if (scoreboardEditAction.isBlue()) {
  ...
  private void _calculateGeneralPoints(SimpleIntegerProperty generalPoints, int points, int penalties) {
    int newGeneralPoints = points + ((getMatchWorker().isGoldenPointWorking() && !this.gamJeomShowPointsOnGoldenPoint && penalties <= 1) ? 0 : penalties);
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    if (this.allowChangeRoundCentiSeconds.booleanValue()) {
      this.roundTimeMinutes.addListener(new ChangeListener<Number>() {
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
              final boolean show = (newValue != null && newValue.intValue() == 0 && CommonScoreboardEditorController.this.roundTimeSeconds.intValue() <= 9);
              Platform.runLater(new Runnable() {
                    public void run() {
                      CommonScoreboardEditorController.this.txtRoundCentiseconds.setVisible(show);
                      CommonScoreboardEditorController.this.lblCentisecondsPoint.setVisible(show);
                    }
                  });
            }
          });
      this.roundTimeSeconds.addListener(new ChangeListener<Number>() {
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
              final boolean show = (newValue != null && CommonScoreboardEditorController.this.roundTimeMinutes.intValue() == 0 && newValue.intValue() <= 9);
              Platform.runLater(new Runnable() {
                    public void run() {
                      CommonScoreboardEditorController.this.txtRoundCentiseconds.setVisible(show);
                      CommonScoreboardEditorController.this.lblCentisecondsPoint.setVisible(show);
      ...
      this.txtRoundCentiseconds.focusedProperty().addListener((ChangeListener)new CustomTimeLostFocusListener((TextField)this.txtRoundCentiseconds, "00"));
      this.txtRoundCentiseconds.textProperty().bindBidirectional((Property)this.roundTimeCentiseconds, (StringConverter)new NumberStringConverter());
    } 
    ...
    this.lblBlueNearMissHits.textProperty().bindBidirectional((Property)this.blueGoldenPointNearMissHits, (StringConverter)new NumberStringConverter());
    this.lblRedNearMissHits.textProperty().bindBidirectional((Property)this.redGoldenPointNearMissHits, (StringConverter)new NumberStringConverter());
    this.bluePenalties.addListener(this.bluePenaltiesListener);
    this.redPenalties.addListener(this.redPenaltiesListener);
    this.bluePoints.addListener(this.bluePointsListener);
    this.redPoints.addListener(this.redPointsListener);
  ...
  private final ChangeListener<Number> bluePointsListener = new ChangeListener<Number>() {
      public void changed(ObservableValue<? extends Number> observableValue, Number number, Number newPoints) {
        CommonScoreboardEditorController.this._calculateBlueGeneralPoints();
      }
    };
  
  private final ChangeListener<Number> redPointsListener = new ChangeListener<Number>() {
      public void changed(ObservableValue<? extends Number> observableValue, Number number, Number newPoints) {
        CommonScoreboardEditorController.this._calculateRedGeneralPoints();
      }
    };
  ...
  private final ChangeListener<Number> bluePenaltiesListener = new ChangeListener<Number>() {
      public void changed(ObservableValue<? extends Number> observableValue, Number number, final Number t1) {
        Platform.runLater(new Runnable() {
              public void run() {
                CommonScoreboardEditorController.this.lblBluePenalties.setText("" + t1);
              }
            });
        CommonScoreboardEditorController.this._calculateRedGeneralPoints();
      }
  ...
  private final ChangeListener<Number> redPenaltiesListener = new ChangeListener<Number>() {
      public void changed(ObservableValue<? extends Number> observableValue, Number number, final Number t1) {
        Platform.runLater(new Runnable() {
              public void run() {
                CommonScoreboardEditorController.this.lblRedPenalties.setText("" + t1);
              }
            });
        CommonScoreboardEditorController.this._calculateBlueGeneralPoints();
      }
