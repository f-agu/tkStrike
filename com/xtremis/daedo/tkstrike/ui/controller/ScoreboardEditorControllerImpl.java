...
@Component
public class ScoreboardEditorControllerImpl extends CommonScoreboardEditorController<NetworkConfigurationEntry, MatchWorker>
	...
	public void addRedSpinningKickPoint() {
		this.scoreboardEditActions.add(new ScoreboardEditAction(false, ScoreboardAction.BODY_TECH_POINT, this.rulesEntry
				.getParaSpinningKickPoints()));
	...
	void _doAfterOnWindowShowEvent() {
		...
		this.btPARAResetTOBlue.setVisible((matchConfigurationEntry.isParaTkdMatch() && ! getMatchWorker().bluePARATimeOutQuotaProperty().getValue()
				.booleanValue()));
		this.btPARAResetTORed.setVisible((matchConfigurationEntry.isParaTkdMatch() && ! getMatchWorker().redPARATimeOutQuotaProperty().getValue()
				.booleanValue()));
	}
	...
	private void validateChangesAppliedIfVideoRequested() {
		if(this.blueVideoRequested || this.redVideoRequested)
			if(this.blueVideoRequested) {
				this.matchWorker.blueVideoRequestResult().setValue(this.changesApplied ? MatchWorker.VideoRequestResult.ACCEPTED
						: MatchWorker.VideoRequestResult.REJECTED);
			} else {
				this.matchWorker.redVideoRequestResult().setValue(this.changesApplied ? MatchWorker.VideoRequestResult.ACCEPTED
						: MatchWorker.VideoRequestResult.REJECTED);
			}
		if((this.blueVideoRequested || this.redVideoRequested) && ! this.changesApplied)
			if(this.blueVideoRequested) {
				showMonologInfoDialog(getMessage("title.default.info"), getMessage("message.removeVideoQuota", new String[] {getMessage(
						"label.blue")}));
	...
	public void doChangeRoundTime() {
		...
		this.lblRoundTime.setText(minutesOrSecondsFormat.format(this.roundTimeMinutes.get()) + ":" + minutesOrSecondsFormat
				.format(this.roundTimeSeconds.get()) + (this.txtRoundCentiseconds.isVisible() ? ("." + this.roundTimeCentiseconds.intValue()) : ""));
	}
	...
	void _initializeRoundAndTime() {
		...
		if(rounds > 0) {
			for(int i = 1; i <= rounds; i++) {
				...
				this.pnRoundSelection.getChildren().addAll(new Node[] {toggleButton});
			}
			if(goldenPoint) {
		...
		if(getMatchWorker().isGoldenPointEnabled() && getMatchWorker().isGoldenPointWorking()) {
			this.round.set( - 1);
			this.lblRoundTime.setText(getMatchWorker().roundCountdownCurrentTimeAsStringProperty().get());
