    ...
    TkStrikeExecutors.executeInThreadPool(new Runnable() {
          public void run() {
            try {
              ...
              final TkStrikeHasNewVersionResponseDto tkStrikeHasNewVersionResponseDto = ConfigurationSoftwareUpdateController.this.tkStrikeSoftwareUpdatesClient.hasNewVersion();
                ...
                Platform.runLater(new Runnable() {
                      public void run() {
                        if (tkStrikeHasNewVersionResponseDto.getHasNewVersion().booleanValue() && tkStrikeHasNewVersionResponseDto
                          ...
                          String newVersion = tkStrikeHasNewVersionResponseDto.getTkStrikeVersionBuildDto().getVersion() + " (" + tkStrikeHasNewVersionResponseDto.getTkStrikeVersionBuildDto().getBuildNumber() + ")";
                          for (TkStrikeOSReleaseInfoDto tkStrikeOSReleaseInfoDto : tkStrikeHasNewVersionResponseDto.getTkStrikeVersionBuildDto().getTkStrikeOSReleaseInfoDtos()) {
                            ...
                            lblVersion.getStyleClass().addAll((Object[])new String[] { "label-subTitle" });
                            ...
                            vBox.onMouseClickedProperty().set(new EventHandler<MouseEvent>() {
                                  public void handle(MouseEvent mouseEvent) {
                                    if (mouseEvent.getClickCount() == 1)
                                      try {
                                        ConfigurationSoftwareUpdateController.this.getHostServices().showDocument(tkStrikeOSReleaseInfoDto.getDownloadURL());
                                      } catch (Exception e) {
                                        e.printStackTrace();
                                      }  
                                  }
              ...
              Platform.runLater(new Runnable() {
                    public void run() {
                      ConfigurationSoftwareUpdateController.this.showErrorDialog(ConfigurationSoftwareUpdateController.this.getMessage("title.default.error"), ConfigurationSoftwareUpdateController.this.getMessage("message.error.softwareUpdates"));
                      ConfigurationSoftwareUpdateController.this.pnContainer.setVisible(true);
                      ConfigurationSoftwareUpdateController.this.pi.setVisible(false);
                    }
                  });
