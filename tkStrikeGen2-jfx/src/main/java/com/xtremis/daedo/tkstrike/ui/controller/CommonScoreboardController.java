...
public abstract class CommonScoreboardController<MW extends CommonMatchWorker> extends CommonTkStrikeBaseController {
		...
		public void changed(ObservableValue<? extends Number> observableValue, Number number, final Number t1) {
			Platform.runLater(new Runnable() {
				...
				public void run() {
					...
					if(CommonScoreboardController.this.showTotalPenaltiesOnGoldenPoint.booleanValue() && CommonScoreboardController.this
							.getMatchWorker().isGoldenPointWorking())
						if(CommonScoreboardController.this.isBlueOnLeft()) {
							CommonScoreboardController.this.lblLeftTotalPenalties.setText("" + CommonScoreboardController.this.getMatchWorker()
									.getBlueTotalPenalties());
						} else {
							CommonScoreboardController.this.lblRightTotalPenalties.setText("" + CommonScoreboardController.this.getMatchWorker()
									.getRedTotalPenalties());
						}
				}
			});
		}
			...
			Platform.runLater(new Runnable() {
				...
				public void run() {
					...
					if(MatchVictoryCriteria.CONVENTIONAL.equals(CommonScoreboardController.this.getMatchWorker().getMatchVictoryCriteria())) {
						CommonScoreboardController.this.clearImpactInScore(CommonScoreboardController.this.pnLeftLastImpact);
						CommonScoreboardController.this.clearImpactInScore(CommonScoreboardController.this.pnRightLastImpact);
	...
	public final void onWindowShowEvent() {
		...
		if(this.currentExternalConfig != null) {
			...
			this.pnLeftAthlete.getStyleClass().removeAll(new String[] {"sb-blue-radial", "sb-red-radial", "sb-blue-plain", "sb-red-plain"});
			pnLeftFlag().getChildren().clear();
			this.pnRightAthlete.getStyleClass().removeAll(new String[] {"sb-blue-radial", "sb-red-radial", "sb-blue-plain", "sb-red-plain"});
			pnRightFlag().getChildren().clear();
			if(isBlueOnLeft()) {
				this.pnLeftAthlete.getStyleClass().addAll(new String[] {getBlueBackgroundStyle()});
				this.pnRightAthlete.getStyleClass().addAll(new String[] {getRedBackgroundStyle()});
			} else {
				this.pnLeftAthlete.getStyleClass().addAll(new String[] {getRedBackgroundStyle()});
				this.pnRightAthlete.getStyleClass().addAll(new String[] {getBlueBackgroundStyle()});
			}
		}
		bindUIControls();
		...
		if(isBlueOnLeft()) {
			...
			if(MatchVictoryCriteria.BESTOF3.equals(getMatchWorker().getMatchVictoryCriteria())) {
				...
				Platform.runLater(new Runnable() {
					...
					@Override
					public void run() {
						((StackPane)CommonScoreboardController.this.pnLeftLastImpact.getChildren().get(0)).getChildren().add(
								new CommonScoreboardController.BestOf3SupLeadPane(CommonScoreboardController.this
										.getMatchWorker().bestOf3CurrentRoundPartialWinner(), true));
						((StackPane)CommonScoreboardController.this.pnRightLastImpact.getChildren().get(0)).getChildren().add(
								new CommonScoreboardController.BestOf3SupLeadPane(CommonScoreboardController.this
										.getMatchWorker().bestOf3CurrentRoundPartialWinner(), false));
						((StackPane)CommonScoreboardController.this.pnLeftLastImpact.getChildren().get(1)).getChildren().add(
								new CommonScoreboardController.BestOf3HitsPane(CommonScoreboardController.this.getMatchWorker()
										.blueNearMissHitsProperty()));
						((StackPane)CommonScoreboardController.this.pnRightLastImpact.getChildren().get(1)).getChildren().add(
								new CommonScoreboardController.BestOf3HitsPane(CommonScoreboardController.this.getMatchWorker()
										.redNearMissHitsProperty()));
					}
				});
			} else {
	...
	private void clearImpactInScore(Pane impactPane) {
		...
		impactPane.getChildren().get(0).setUserData(null);
		((Pane)impactPane.getChildren().get(1)).getChildren().clear();
		impactPane.getChildren().get(1).setUserData(null);
	}
	...
	private Timeline addFlash(StackPane impactPane, Timeline flash) {
		flash = new Timeline(new KeyFrame[] {new KeyFrame(Duration.millis(600.0D), new KeyValue[] {new KeyValue((WritableValue)impactPane
				.visibleProperty(), Boolean.valueOf(false))}), new KeyFrame(Duration.millis(900.0D), new KeyValue[] {new KeyValue(
						(WritableValue)impactPane.visibleProperty(), Boolean.valueOf(true))})});
	...
	public final void afterPropertiesSet() throws Exception {
		...
		getAppStatusWorker().bestOf3SuperiorityRoundWinnerProperty().addListener(new ChangeListener() {
			...
			public void changed(ObservableValue observable, Object oldValue, final Object newValue) {
				if(newValue != null && newValue instanceof MatchWinner &&
						MatchVictoryCriteria.BESTOF3.equals(CommonScoreboardController.this.getMatchWorker().getMatchVictoryCriteria())
						&& MatchWinner.TIE
								.equals(CommonScoreboardController.this.getMatchWorker().getRoundWinner(Integer.valueOf(
										CommonScoreboardController.this.getMatchWorker().getCurrentRound()))))
					Platform.runLater(new Runnable() {
		...
		if(TkStrikeScoreboardGraphicDetailType.LOW_GRAPHIC_DETAIL.equals(this.scoreboardGraphicDetailType)) {
			Timeline flashLeftPoints = new Timeline(new KeyFrame[] {new KeyFrame(Duration.millis(600.0D), new KeyValue[] {new KeyValue(
					(WritableValue)this.txtLeftPoints.visibleProperty(), Boolean.valueOf(false))}), new KeyFrame(Duration.millis(900.0D),
							new KeyValue[] {new KeyValue((WritableValue)this.txtLeftPoints.visibleProperty(), Boolean.valueOf(true))})});
			flashLeftPoints.setDelay(Duration.seconds(0.5D));
			flashLeftPoints.setCycleCount(3);
			flashLeftPoints.setAutoReverse(false);
			Timeline flashRightPoints = new Timeline(new KeyFrame[] {new KeyFrame(Duration.millis(600.0D), new KeyValue[] {new KeyValue(
					(WritableValue)this.txtRightPoints.visibleProperty(), Boolean.valueOf(false))}), new KeyFrame(Duration.millis(900.0D),
							new KeyValue[] {new KeyValue((WritableValue)this.txtRightPoints.visibleProperty(), Boolean.valueOf(true))})});
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
				if(newValue.booleanValue()) {
					...
					if(MatchVictoryCriteria.BESTOF3.equals(CommonScoreboardController.this.getMatchWorker().getMatchVictoryCriteria())
							&& MatchStatusId.ROUND_FINISHED.equals(CommonScoreboardController.this.getMatchWorker().getCurrentMatchStatus()))
						Platform.runLater(new Runnable() {
		...
		getMatchWorker().currentMatchStatusProperty().addListener(new ChangeListener<MatchStatusId>() {
			...
			@Override
			public void changed(ObservableValue<? extends MatchStatusId> observableValue, final MatchStatusId prevMatchStatusId,
					final MatchStatusId newMatchStatus) {
					...
					Platform.runLater(new Runnable() {
						...
						public void run() {
							...
							try {
								...
								CommonScoreboardController.this.pnRest().setVisible((MatchStatusId.ROUND_FINISHED.equals(newMatchStatus)
										|| MatchStatusId.ROUND_PARA_TIMEOUT_WORKING.equals(newMatchStatus)));
								boolean showRoundWinsInfo = (MatchVictoryCriteria.BESTOF3.equals(CommonScoreboardController.this.getMatchWorker()
										.getMatchVictoryCriteria()) && (MatchStatusId.ROUND_FINISHED.equals(newMatchStatus)
												|| MatchStatusId.ROUND_PARA_TIMEOUT_WORKING.equals(newMatchStatus)));
								...
								CommonScoreboardController.this.lblRest().setText(CommonScoreboardController.this.getMessage(
										MatchStatusId.ROUND_PARA_TIMEOUT_WORKING.equals(newMatchStatus) ? "message.matchPaused"
												: "message.restTime"));
								CommonScoreboardController.this.pnTimeout().setVisible((MatchStatusId.ROUND_PAUSED.equals(newMatchStatus)
										|| MatchStatusId.NETWORK_ERROR.equals(newMatchStatus)));
								CommonScoreboardController.this.pnNetworkError().setVisible((MatchStatusId.NETWORK_ERROR.equals(newMatchStatus)
								...
								if(networkErrorCause != null) {
									...
									if(networkErrorCause.isBlue()) {
										if(networkErrorCause.isBody()) {
											CommonScoreboardController.this.ivErrorCause.setImage(new Image(getClass().getResourceAsStream(
													"/images/blueBody.png")));
										} else {
											CommonScoreboardController.this.ivErrorCause.setImage(new Image(getClass().getResourceAsStream(
													"/images/blueHead.png")));
									...
									CommonScoreboardController.this.lblNetworkError1.getStyleClass().removeAll(new String[] {"label-blue",
											"label-red"});
									CommonScoreboardController.this.lblNetworkError1.getStyleClass().addAll(new String[] {labelStyle});
									CommonScoreboardController.this.lblNetworkError1.setText(labelMessage);
									CommonScoreboardController.this.lblNetworkError2.setText(CommonScoreboardController.this.getMessage(
											"label.networkError.general"));
								}
								...
								if(MatchStatusId.MATCH_FINISHED.equals(newMatchStatus)) {
									if(CommonScoreboardController.this.getMatchWorker().isParaTkdMatch()
											? CommonScoreboardController.this.pnGoldenPointTieBreakerPARA.isVisible()
											: CommonScoreboardController.this.pnGoldenPointTieBreaker.isVisible()) {
								...
								CommonScoreboardController.this.showWinnerProperty.setValue(Boolean.valueOf((showWinner
										|| (MatchStatusId.MATCH_NEEDS_CONFIRM_FINAL_DECISION.equals(newMatchStatus)
												&& CommonScoreboardController.this.showWinnerInfoWhenGoldenPointFinish.booleanValue()))));
								...
								if(newMatchStatus.equals(MatchStatusId.MATCH_FINISHED)) {
									...
									if(newMatchStatus.equals(MatchStatusId.WAITING_4_START_GOLDENPOINT) || CommonScoreboardController.this
										...
										if(CommonScoreboardController.this.isBlueOnLeft()) {
											CommonScoreboardController.this.lblLeftPenalties.setText("" + CommonScoreboardController.this
													.getMatchWorker().getBlueGoldenPointPenalties());
											CommonScoreboardController.this.lblRightPenalties.setText("" + CommonScoreboardController.this
													.getMatchWorker().getRedGoldenPointPenalties());
										} else {
											CommonScoreboardController.this.lblLeftPenalties.setText("" + CommonScoreboardController.this
													.getMatchWorker().getRedGoldenPointPenalties());
											CommonScoreboardController.this.lblRightPenalties.setText("" + CommonScoreboardController.this
									...
									if(newMatchStatus.equals(MatchStatusId.WAITING_4_START_GOLDENPOINT)) {
										...
										CommonScoreboardController.this.pnRightHits().getChildren().clear();
										if(CommonScoreboardController.this.isBlueOnLeft()) {
											CommonScoreboardController.this.txtLeftPoints().setText("" + CommonScoreboardController.this
													.getMatchWorker().getBlueGeneralPoints());
											CommonScoreboardController.this.txtRightPoints().setText("" + CommonScoreboardController.this
													.getMatchWorker().getRedGeneralPoints());
										} else {
											CommonScoreboardController.this.txtLeftPoints().setText("" + CommonScoreboardController.this
													.getMatchWorker().getRedGeneralPoints());
											CommonScoreboardController.this.txtRightPoints().setText("" + CommonScoreboardController.this
										...
										if(CommonScoreboardController.this.showTotalPenaltiesOnGoldenPoint.booleanValue())
											Platform.runLater(new Runnable() {
												...
												public void run() {
													...
													if(CommonScoreboardController.this.isBlueOnLeft()) {
														CommonScoreboardController.this.lblLeftTotalPenalties.setText(""
																+ CommonScoreboardController.this.getMatchWorker().getBlueTotalPenalties());
														CommonScoreboardController.this.lblRightTotalPenalties.setText(""
																+ CommonScoreboardController.this.getMatchWorker().getRedTotalPenalties());
													} else {
														CommonScoreboardController.this.lblLeftTotalPenalties.setText(""
																+ CommonScoreboardController.this.getMatchWorker().getRedTotalPenalties());
														CommonScoreboardController.this.lblRightTotalPenalties.setText(""
									...
									CommonScoreboardController.this.clearImpactInScore(CommonScoreboardController.this.pnLeftLastImpact);
									CommonScoreboardController.this.clearImpactInScore(CommonScoreboardController.this.pnRightLastImpact);
									CommonScoreboardController.this.getMatchWorker().bluePenaltiesProperty().removeListener(
											CommonScoreboardController.this.penaltiesLeftListener);
									CommonScoreboardController.this.getMatchWorker().bluePenaltiesProperty().removeListener(
											CommonScoreboardController.this.penaltiesRightListener);
									CommonScoreboardController.this.getMatchWorker().redPenaltiesProperty().removeListener(
											CommonScoreboardController.this.penaltiesRightListener);
									CommonScoreboardController.this.getMatchWorker().redPenaltiesProperty().removeListener(
											CommonScoreboardController.this.penaltiesLeftListener);
									if(CommonScoreboardController.this.isBlueOnLeft()) {
										CommonScoreboardController.this.getMatchWorker().blueGoldenPointPenaltiesProperty().addListener(
												CommonScoreboardController.this.penaltiesLeftListener);
										CommonScoreboardController.this.getMatchWorker().redGoldenPointPenaltiesProperty().addListener(
												CommonScoreboardController.this.penaltiesRightListener);
									} else {
										CommonScoreboardController.this.getMatchWorker().blueGoldenPointPenaltiesProperty().addListener(
												CommonScoreboardController.this.penaltiesRightListener);
		...
		this.lblDoctor.visibleProperty().bind(getMatchWorker().doctorInRoundProperty());
		this.showWinnerProperty.addListener(new ChangeListener<Boolean>() {

			@Override
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
				if(CommonScoreboardController.this.getMatchWorker().isParaTkdMatch() ? CommonScoreboardController.this.pnGoldenPointTieBreakerPARA
						.isVisible() : CommonScoreboardController.this.pnGoldenPointTieBreaker.isVisible())
					...
					if(CommonScoreboardController.this.getMatchWorker().isParaTkdMatch() ? CommonScoreboardController.this.pnGoldenPointTieBreakerPARA
						...
						Platform.runLater(new Runnable() {
							...
							public void run() {
								CommonScoreboardController.this.pnShowWinner().setVisible(false);
								if(CommonScoreboardController.this.getMatchWorker().isParaTkdMatch()) {
									CommonScoreboardController.this.putWinnerInfo(CommonScoreboardController.this.lblMatchWinnerTitleGDPTBPARA,
											CommonScoreboardController.this.lblFinalDecissionGDPTBPARA,
											CommonScoreboardController.this.lblWinnerGDPTBPARA,
											CommonScoreboardController.this.ivWinnerFlagGDPTBPARA);
								} else {
									CommonScoreboardController.this.putWinnerInfo(CommonScoreboardController.this.lblMatchWinnerTitleGDPTB,
											CommonScoreboardController.this.lblFinalDecissionGDPTB, CommonScoreboardController.this.lblWinnerGDPTB,
											CommonScoreboardController.this.ivWinnerFlagGDPTB);
								}
							}
						});
						return;
		...
		getMatchWorker().bestOf3RoundSuperiorityOnScoreboard().addListener(new ChangeListener<Boolean>() {
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
				...
				Platform.runLater(new Runnable() {
					...
					@Override
					public void run() {
						CommonScoreboardController.this.lblMatchTitleRoundSUPBestOf3.setText(CommonScoreboardController.this.lblMatchConfig
								.getText());
						CommonScoreboardController.this.lblRoundSUPBestOf3.setText("ROUND " + CommonScoreboardController.this.txtCurrentRound
								.getText());
						BestOf3RoundSuperiority bestOf3RoundSuperiority = CommonScoreboardController.this.getMatchWorker()
								.getCurrentBestOf3RoundSuperiority();
						if(bestOf3RoundSuperiority != null) {
							...
							if(MatchWinner.TIE.equals(currentRoundWinner)) {
								CommonTkStrikeBaseController.logger.info(
										"bestOf3RoundSuperiorityOnScoreboard superiority on TIE get from AppStatusWorker");
								currentRoundWinner = (MatchWinner)CommonScoreboardController.this.getAppStatusWorker()
										.bestOf3SuperiorityRoundWinnerProperty().get();
							}
							CommonScoreboardController.this.updateBestOf3RoundSuperiorityWinnerInfo(currentRoundWinner);
							CommonScoreboardController.this.lblBlueRoundSUPBestOf3TurningSpinning.setText("" + bestOf3RoundSuperiority
									.getBlueTurningSpinning());
							CommonScoreboardController.this.lblRedRoundSUPBestOf3TurningSpinning.setText("" + bestOf3RoundSuperiority
									.getRedTurningSpinning());
			...
			public void changed(ObservableValue<? extends FinalDecision> observable, FinalDecision oldValue, FinalDecision newValue) {
				if(newValue != null) {
					...
					if(FinalDecision.PTG.equals(newValue) && (MatchStatusId.ROUND_WORKING
						...
						CommonTkStrikeBaseController.logger.info("matchFinalDecisionProperty changed FinalDecision " + newValue + " MatchSatus: "
								+ currentMatchStatus);
						Platform.runLater(new Runnable() {
				...
				Platform.runLater(new Runnable() {
					...
					public void run() {
						if(Boolean.TRUE.equals(newValue)) {
							...
							CommonTkStrikeBaseController.logger.info("BestOf3 round  " + CommonScoreboardController.this.getMatchWorker()
									.getCurrentRound() + " won by point gap to " + matchWinner);
							CommonScoreboardController.this.lblPointGapWinner.getStyleClass()
									.removeAll(new String[] {"sb-blue", "sb-red"});
							CommonScoreboardController.this.lblPointGapWinner.getStyleClass()
									.addAll(new String[] {MatchWinner.BLUE.equals(matchWinner) ? "sb-blue" : "sb-red"});
							CommonScoreboardController.this.lblPointGapWinner.setText(MatchWinner.BLUE.equals(matchWinner)
									? CommonScoreboardController.this
											.getMessage("label.blue").toUpperCase()
									: CommonScoreboardController.this
											.getMessage("label.red").toUpperCase());
						}
						boolean show = (newValue != null && newValue.booleanValue() && ! MatchStatusId.ROUND_FINISHED.equals(
								CommonScoreboardController.this.getMatchWorker().getCurrentMatchStatus())
								&& ! MatchStatusId.ROUND_PARA_TIMEOUT_WORKING.equals(CommonScoreboardController.this.getMatchWorker()
					...
					Platform.runLater(new Runnable() {
						...
						public void run() {
							if( ! MatchWinner.TIE.equals(newValue)) {
								CommonScoreboardController.this.lblPointGapWinner.getStyleClass().removeAll(new String[] {"sb-blue", "sb-red"});
								CommonScoreboardController.this.lblPointGapWinner.getStyleClass().addAll(new String[] {MatchWinner.BLUE.equals(
										newValue) ? "sb-blue" : "sb-red"});
								CommonScoreboardController.this.lblPointGapWinner.setText(MatchWinner.BLUE.equals(newValue)
										? CommonScoreboardController.this
												.getMessage("label.blue").toUpperCase()
										: CommonScoreboardController.this.getMessage("label.red").toUpperCase());
							} else {
								CommonScoreboardController.this.showOrHidePointGap(true, false);
							}
						}
		...
		getMatchWorker().addListener(new CommonMatchWorker.HitEventValidatorListener() {
									...
									if((hitEventType.equals(combatHit.getHitEventType()) && ! validated && combatHit
											.isJudgesTechValidated()) || (validated && combatHit
													...
													.getJudge1Status().equals(HitJudgeStatus.NOT_VALIDATED) && (combatHit
															.getJudge2Status().equals(HitJudgeStatus.NOT_VALIDATED) || combatHit.getJudge2Status()
																	.equals(HitJudgeStatus.NOT_ENABLED)) && (combatHit
																			.getJudge3Status().equals(HitJudgeStatus.NOT_VALIDATED) || combatHit
																					.getJudge3Status().equals(HitJudgeStatus.NOT_ENABLED))))
										pos = n;
									n++;
								}
							}
					...
					Platform.runLater(new Runnable() {
						...
						public void run() {
							try {
								...
								if((CommonScoreboardController.this.isBlueOnLeft() && newValue.isBlue()) || ( ! CommonScoreboardController.this
										.isBlueOnLeft() && ! newValue.isBlue())) {
									thePane = CommonScoreboardController.this.pnLeftHits();
								} else {
									thePane = CommonScoreboardController.this.pnRightHits();
									isRight = true;
								}
								final TkStrikeCombatHit combatHit = new TkStrikeCombatHit(CommonScoreboardController.this.scoreboardGraphicDetailType,
										newValue
												.getHitTimestamp(), newValue.getHitEventType(), newValue.getHitValue().intValue(), newValue
														.getJudgesEnabled(), CommonScoreboardController.this.getHitsControlDefaultFontSize(), Double
																.valueOf(
																		CommonScoreboardController.this.getHitsControlHeight()), Double.valueOf(
																				CommonScoreboardController.this.getHitsControlSpacing()), Double
																						.valueOf(
																								CommonScoreboardController.this
																										.getHitsControlJudgesWidth()), Double.valueOf(
																												CommonScoreboardController.this
																														.getHitsControlHitWidth()),
										isRight,
										newValue.isBackupSystemEnabled());
								...
								TkStrikeCombatHit otherHit = thePane.getChildren().stream()
										.filter(t -> combatHit.getHitTimestamp() == ((TkStrikeCombatHit)t).getHitTimestamp())
										.map(n -> (TkStrikeCombatHit)n)
										.findAny()
										.orElse(null);
								if(otherHit != null)
								...
								if(((newValue.getHitValue().intValue() > 100 && newValue.getHitEventType().equals(HitEventType.HEAD)) || (newValue
										.getHitValue().intValue() > 100 && newValue.getHitEventType().equals(HitEventType.BODY)) || newValue
												.getHitEventType().equals(HitEventType.PUNCH) || newValue.getHitEventType().equals(
														HitEventType.SPECIAL_HEAD)) &&
			...
			public void removeGoldenPointNearMissHit(final boolean blue) {
				Platform.runLater(new Runnable() {
					...
					public void run() {
						Pane thePane = null;
						if((CommonScoreboardController.this.isBlueOnLeft() && blue) || ( ! CommonScoreboardController.this.isBlueOnLeft()
								&& ! blue)) {
							thePane = CommonScoreboardController.this.pnLeftHits();
						} else {
							thePane = CommonScoreboardController.this.pnRightHits();
			...
			public void addGoldenPointNearMissHit(final boolean blue) {
				Platform.runLater(new Runnable() {
					...
					public void run() {
						...
						TkStrikeCombatHit combatHit = new TkStrikeCombatHit(CommonScoreboardController.this.scoreboardGraphicDetailType, System
								.currentTimeMillis(), HitEventType.BODY, 6, 3, CommonScoreboardController.this.getHitsControlDefaultFontSize(), Double
										.valueOf(CommonScoreboardController.this.getHitsControlHeight()), Double.valueOf(
												CommonScoreboardController.this.getHitsControlSpacing()), Double.valueOf(
														CommonScoreboardController.this.getHitsControlJudgesWidth()), Double.valueOf(
																CommonScoreboardController.this.getHitsControlHitWidth()), isRight,
								CommonScoreboardController.this.getMatchWorker().isBackupSystemEnabled());
						thePane.getChildren().addAll(new Node[] {combatHit});
					}
				});
			}
				...
				Platform.runLater(new Runnable() {
					...
					public void run() {
						try {
							...
							if(combatHit == null) {
								combatHit = new TkStrikeCombatHit(CommonScoreboardController.this.scoreboardGraphicDetailType, newValue
										.getHitTimestamp(), newValue.getHitEventType(), newValue.getHitValue().intValue(), newValue
												.getJudgesEnabled(), CommonScoreboardController.this.getHitsControlDefaultFontSize(), Double.valueOf(
														CommonScoreboardController.this.getHitsControlHeight()), Double.valueOf(
																CommonScoreboardController.this.getHitsControlSpacing()), Double.valueOf(
																		CommonScoreboardController.this.getHitsControlJudgesWidth()), Double.valueOf(
																				CommonScoreboardController.this.getHitsControlHitWidth()), isRight,
									...
									combatHit = new TkStrikeCombatHit(CommonScoreboardController.this.scoreboardGraphicDetailType, newValue
											.getHitTimestamp(), newValue.getHitEventType(), newValue.getHitValue().intValue(), newValue
													.getJudgesEnabled(), CommonScoreboardController.this.getHitsControlDefaultFontSize(), Double
															.valueOf(CommonScoreboardController.this.getHitsControlHeight()), Double.valueOf(
																	CommonScoreboardController.this.getHitsControlSpacing()), Double.valueOf(
																			CommonScoreboardController.this.getHitsControlJudgesWidth()), Double
																					.valueOf(CommonScoreboardController.this
																							.getHitsControlHitWidth()), isRight, newValue
																									.isBackupSystemEnabled(), judge1Status,
											judge2Status, judge3Status);
									thePane.getChildren().addAll(new Node[] {combatHit});
								}
							}
							if(((newValue.getHitValue().intValue() > 100 && newValue.getHitEventType().equals(HitEventType.HEAD)) || (newValue
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, final Boolean newValue) {
				Platform.runLater(new Runnable() {
					...
					public void run() {
						...
						if(newValue.booleanValue() && CommonScoreboardController.this.getMatchWorker().bluePARATimeOutQuotaValueProperty()
								.get() > 0) {
	...
	private void updateBestOf3RoundSuperiorityWinnerInfo(MatchWinner roundWinner) {
		...
		this.lblWinnerRoundSUPBestOf3.getStyleClass().removeAll(new String[] {"sb-blue", "sb-red"});
		...
		if(MatchWinner.BLUE.equals(roundWinner)) {
			if(isBlueOnLeft()) {
				...
				this.lblWinnerRoundSUPBestOf3.getStyleClass().addAll(new String[] {"sb-blue"});
			} else {
			...
			if(isBlueOnLeft()) {
				...
				this.lblWinnerRoundSUPBestOf3.getStyleClass().addAll(new String[] {"sb-red"});
			} else {
				boolean showFlag = StringUtils.isNotBlank(this.txtLeftAbbr.getText());
				winnerName = txtLeftName().getText() + (showFlag ? (" (" + this.txtLeftAbbr.getText() + ")") : "");
				if(showFlag && pnLeftFlag().getChildren() != null && pnLeftFlag().getChildren().size() > 0) {
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, final Boolean newValue) {
				Platform.runLater(new Runnable() {
					...
					public void run() {
						...
						if(CommonScoreboardController.this.getMatchWorker().isParaTkdMatch()) {
							CommonScoreboardController.this.putWinnerInfo(CommonScoreboardController.this.lblMatchWinnerTitleGDPTBPARA,
									CommonScoreboardController.this.lblFinalDecissionGDPTBPARA, CommonScoreboardController.this.lblWinnerGDPTBPARA,
									CommonScoreboardController.this.ivWinnerFlagGDPTBPARA);
							CommonScoreboardController.this.updateTieBreakerPenalties();
							...
							CommonScoreboardController.this.createRoundWinnersOnTieBreaker(CommonScoreboardController.this.lblBlueGPTBRounds,
									CommonScoreboardController.this.lblRedGPTBRounds);
							CommonScoreboardController.this.updateTieBreakerPenalties();
							CommonScoreboardController.this.putWinnerInfo(CommonScoreboardController.this.lblMatchWinnerTitleGDPTB,
									CommonScoreboardController.this.lblFinalDecissionGDPTB, CommonScoreboardController.this.lblWinnerGDPTB,
									CommonScoreboardController.this.ivWinnerFlagGDPTB);
	...
	private void putWinnerInfo(Label matchInfo, Label finalDecission, Label winner, ImageView ivWinner) {
		...
		boolean tie = true;
		winner.getStyleClass().removeAll(new String[] {"sb-blue", "sb-red"});
		if(matchWinner.equals(MatchWinner.BLUE)) {
			if(isBlueOnLeft()) {
				...
				winner.getStyleClass().addAll(new String[] {"sb-blue"});
			} else {
				boolean showFlag = StringUtils.isNotBlank(this.txtRightAbbr.getText());
				winnerName = txtRightName().getText() + (showFlag ? (" (" + this.txtRightAbbr.getText() + ")") : "");
				if(showFlag && pnRightFlag().getChildren() != null && pnRightFlag().getChildren().size() > 0) {
			...
			if(isBlueOnLeft()) {
				...
				winner.getStyleClass().addAll(new String[] {"sb-red"});
			} else {
				boolean showFlag = StringUtils.isNotBlank(this.txtLeftAbbr.getText());
				winnerName = txtLeftName().getText() + (showFlag ? (" (" + this.txtLeftAbbr.getText() + ")") : "");
				if(showFlag && pnLeftFlag().getChildren() != null && pnLeftFlag().getChildren().size() > 0) {
	...
	private void _refreshWinnerInfo() {
		if(this.showWinnerProperty.getValue().booleanValue()) {
			Platform.runLater(new Runnable() {
				...
				@Override
				public void run() {
					CommonScoreboardController.this.putWinnerInfo(CommonScoreboardController.this.lblMatchWinnerTitle,
							CommonScoreboardController.this.lblFinalDecission, CommonScoreboardController.this.lblWinner,
							CommonScoreboardController.this.ivWinnerFlag);
					if(MatchVictoryCriteria.BESTOF3.equals(CommonScoreboardController.this.getMatchWorker().getMatchVictoryCriteria())) {
						CommonScoreboardController.this.lblFinalBlueResult.setText("" + CommonScoreboardController.this.getMatchWorker()
								.getBlueRoundsWins());
						CommonScoreboardController.this.lblFinalRedResult.setText("" + CommonScoreboardController.this.getMatchWorker()
								.getRedRoundsWins());
					} else if(CommonScoreboardController.this.lblFinalBlueResult != null
							&& CommonScoreboardController.this.lblFinalRedResult != null) {
						CommonScoreboardController.this.lblFinalBlueResult.setText("" + CommonScoreboardController.this.getMatchWorker()
								.getBlueGeneralPoints());
						CommonScoreboardController.this.lblFinalRedResult.setText("" + CommonScoreboardController.this.getMatchWorker()
								.getRedGeneralPoints());
					}
					...
					CommonScoreboardController.this.pnShowWinner().setVisible( ! CommonScoreboardController.this.getMatchWorker()
							.superiorityByRoundsProperty().get());
				}
			});
		} else {
			pnShowWinner().setVisible(false);
	...
	private Node getNode4Impact(int impactValue) {
		return (impactValue >= 0) ? TkStrikeScoreboardGraphicDetailTypeUtil.getInstance().getNode4BodyImpact(this.scoreboardGraphicDetailType,
				getImpactImageHeight()) :
			...
			public void run() {
				try {
					...
					if( ! CommonScoreboardController.this.getMatchWorker().isBackupSystemEnabled()) {
						...
						impactText.getStyleClass().addAll(CommonScoreboardController.this.getTextImpactStyleClass());
						...
						StackPane.setAlignment(impactText, Pos.CENTER);
	...
	class LastImpactListener implements ChangeListener<Number> {
		...
		public void changed(ObservableValue<? extends Number> observable, Number oldValue, final Number newImpact) {
			if(newImpact.intValue() != 0 && ! MatchVictoryCriteria.BESTOF3.equals(CommonScoreboardController.this.getMatchWorker()
					...
					public void run() {
						...
						if(impacts2Show.size() >= 1) {
							...
							if(node.getUserData() != null && node.getUserData() instanceof Integer)
								CommonScoreboardController.this._showImpact(1, (Pane)pnLastImpact, (Integer)node.getUserData(), false,
										(Timeline)null);
						...
						n = 0;
						CommonScoreboardController.this._showImpact(n, pnLastImpact, Integer.valueOf(newImpact.intValue()), true,
								(CommonScoreboardController.LastImpactListener.this.left.booleanValue() && n == 0)
										? CommonScoreboardController.this.flashLeftImpact1
										: ((CommonScoreboardController.LastImpactListener.this.left.booleanValue() && n == 1)
												? CommonScoreboardController.this.flashLeftImpact2
												: (( ! CommonScoreboardController.LastImpactListener.this.left.booleanValue() && n == 0)
														? CommonScoreboardController.this.flashRightImpact1
														: (( ! CommonScoreboardController.LastImpactListener.this.left.booleanValue() && n == 1)
																? CommonScoreboardController.this.flashRightImpact2
																: null))));
					}
				});
		}
	...
	static class BestOf3SupLeadPane extends BorderPane {
		...
		public BestOf3SupLeadPane(ReadOnlyObjectProperty<MatchWinner> bestOf3CurrentRoundPartialWinner, boolean blue) {
			...
			Circle circle = new Circle(15.0D);
			circle.setStrokeWidth(0.0D);
			circle.setFill(this.COLOR_GREEN);
			Circle circle2 = new Circle(12.0D);
			circle2.setStrokeWidth(4.0D);
			circle2.setStroke(Color.BLACK);
			circle2.setFill(this.COLOR_GREEN);
			StackPane.setAlignment(circle, Pos.CENTER);
			StackPane.setAlignment(circle2, Pos.CENTER);
			this.circleContainer.getChildren().addAll(new Node[] {circle, circle2});
			updateByMatchWinner(bestOf3CurrentRoundPartialWinner.getValue());
			...
			BorderPane.setAlignment(this.circleContainer, Pos.CENTER);
			setCenter(this.circleContainer);
			Label supLeadLabel = new Label("SUP LEAD");
			supLeadLabel.getStyleClass().addAll(new String[] {"bestOf3SpecialLabel"});
			BorderPane.setAlignment(supLeadLabel, Pos.CENTER);
			setTop(supLeadLabel);
		}
	...
	static class BestOf3HitsPane extends BorderPane {
		...
		public BestOf3HitsPane(ReadOnlyIntegerProperty hitsProperty) {
			...
			this.labelHits.getStyleClass().addAll(new String[] {"bestOf3HitsLabel"});
			BorderPane.setAlignment(this.labelHits, Pos.CENTER);
			setCenter(this.labelHits);
