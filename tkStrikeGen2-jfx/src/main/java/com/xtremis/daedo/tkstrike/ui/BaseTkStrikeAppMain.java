...
public abstract class BaseTkStrikeAppMain extends Application {
	...
	public void init() throws Exception {
		...
		this.splashLayout.getChildren().addAll(new Node[] {empty, this.loadProgress, this.progressText});
		this.splashLayout.setStyle(
				"-fx-padding: 10; -fx-background-color: white; -fx-border-width:1; -fx-border-color: black;-fx-background-image: url('" +
			...
			protected Boolean call() throws InterruptedException {
				...
				if(log4jProperties.exists()) {
					...
					if(defaultLog4jPropertiesUrl != null) {
						...
						appenderDefault.setLayout(new PatternLayout("%d{dd/MM/yyyy HH:mm:ss,SSS} - %m%n"));
						appenderDefault.setThreshold(Level.INFO);
						appenderDefault.setImmediateFlush(true);
						appenderDefault.setAppend(true);
						appenderDefault.activateOptions();
						Logger.getRootLogger().addAppender(appenderDefault);
						...
						appenderExternalIntegration.setFile(tkStrikeBaseDirectoriesUtil.getLogBaseDir() + BaseTkStrikeAppMain.this
								.getExternalIntegrationLogFileName());
						appenderExternalIntegration.setMaxBackupIndex(10);
						appenderExternalIntegration.setMaxFileSize("2048KB");
						appenderExternalIntegration.setLayout(new PatternLayout("%d{dd/MM/yyyy HH:mm:ss,SSS} - %m%n"));
						appenderExternalIntegration.setThreshold(Level.INFO);
						...
						loggerExternalIntegration.addAppender(appenderExternalIntegration);
						RollingFileAppender appenderCSVImporter = new RollingFileAppender();
						appenderCSVImporter.setName("CSV_IMPORTER");
						appenderCSVImporter.setFile(tkStrikeBaseDirectoriesUtil.getLogBaseDir() + BaseTkStrikeAppMain.this
								.getCSVImporterLogFileName());
						appenderCSVImporter.setMaxBackupIndex(10);
						appenderCSVImporter.setMaxFileSize("2048KB");
						appenderCSVImporter.setLayout(new PatternLayout("%d{dd/MM/yyyy HH:mm:ss,SSS} - %m%n"));
						appenderCSVImporter.setThreshold(Level.INFO);
						...
						loggerCSVImporter.addAppender(appenderCSVImporter);
					}
				}
				try {
					...
					try {
						ExternalConfigService externalConfigService = BaseTkStrikeAppMain.this.contextAnnotation.getBean(ExternalConfigService.class);
					...
					if(externalConfigEntry != null) {
						if(StringUtils.isNotBlank(externalConfigEntry.getRtBroadcastIp()) &&
							...
							try {
								RtBroadcastSocketClient rtBroadcastSocketClient = BaseTkStrikeAppMain.this.contextAnnotation.getBean(
										RtBroadcastSocketClient.class);
								rtBroadcastSocketClient.connect(externalConfigEntry.getRtBroadcastIp(), Long.valueOf(externalConfigEntry
										.getRtBroadcastPort()), externalConfigEntry.getRtBroadcastRingNumber());
							} catch(Exception exception) {}
						if(wtCompetitionDataProtocol.booleanValue()) {
							updateMessage("WT Ovr UDP validation and connection...");
							WtUDPService wtUDPService = BaseTkStrikeAppMain.this.contextAnnotation.getBean(WtUDPService.class);
					...
					AppStatusWorker appStatusWorker = BaseTkStrikeAppMain.this.contextAnnotation.getBean(AppStatusWorker.class);
					BaseTkStrikeAppMain.this.tkStrikeCommunicationService.addListener(appStatusWorker);
					updateMessage("Network recognition...");
					NetworkConfigurationService networkConfigurationService = BaseTkStrikeAppMain.this.contextAnnotation.getBean(
							NetworkConfigurationService.class);
					INetworkConfigurationEntry currentNetworkConfiguration = networkConfigurationService.getNetworkConfigurationEntry();
					try {
						BaseTkStrikeAppMain.this.tkStrikeCommunicationService.tryToRecognizeWithConfig(currentNetworkConfiguration
								.getNetworkConfigurationDto(), false);
					} catch(Exception e) {
						e.printStackTrace();
					}
				} catch(Exception e) {
		...
		task.stateProperty().addListener(new ChangeListener<Worker.State>() {
			...
			public void changed(ObservableValue<? extends Worker.State> observableValue, Worker.State oldState, Worker.State newState) {
				if(newState == Worker.State.SUCCEEDED) {
					...
					initStage.toFront();
					FadeTransition fadeSplash = new FadeTransition(Duration.seconds(1.2D), BaseTkStrikeAppMain.this.splashLayout);
					...
					fadeSplash.play();
					try {
						initCompletionHandler.complete(task.get());
	...
	private void showMainStage(Boolean initErrors) {
		final TkStrikeMainController tkStrikeMainController = this.contextAnnotation.getBean(getTkStrikeMainControllerClass());
		tkStrikeMainController.setHostServices(getHostServices());
		tkStrikeMainController.setDatabaseInitializedOnStart(initErrors);
		TkStrikeStyleSheetsHelper tkStrikeStyleSheetsHelper = this.contextAnnotation.getBean(TkStrikeStyleSheetsHelper.class);
		...
		PropertySourcesPlaceholderConfigurer propertySourcesPlaceholderConfigurer = this.contextAnnotation.getBean(
