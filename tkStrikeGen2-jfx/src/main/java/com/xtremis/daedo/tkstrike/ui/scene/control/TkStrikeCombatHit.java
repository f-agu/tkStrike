...
public class TkStrikeCombatHit extends HBox {
  ...
  public TkStrikeCombatHit(TkStrikeScoreboardGraphicDetailType scoreboardGraphicDetailType, long hitTimestamp, HitEventType hitEventType, int hitValue, int judgesEnabled, int defaultFontSize, Double controlHeight, Double controlSpacing, Double controlJudgesWidth, Double controlHitWidth, boolean hitFirst, boolean backupSystemEnabled, HitJudgeStatus prevJudge1HitStatus, HitJudgeStatus prevJudge2HitStatus, HitJudgeStatus prevJudge3HitStatus) {
    ...
    txtJ1.getStyleClass().addAll((Object[])new String[] { "df-listHitValues" });
    ...
    if (judgesEnabled >= 2) {
      ...
      txtJ2.getStyleClass().addAll((Object[])new String[] { "df-listHitValues" });
      txtJ2.setStyle("-fx-font-size: " + defaultFontSize + "; -fx-font-weight: bold;");
      this.stJudge2.getChildren().addAll((Object[])new Node[] { (Node)this.ciJudge2, (Node)txtJ2 });
    ...
    if (judgesEnabled >= 3) {
      ...
      txtJ3.getStyleClass().addAll((Object[])new String[] { "df-listHitValues" });
      txtJ3.setStyle("-fx-font-size: " + defaultFontSize + "; -fx-font-weight: bold;");
      this.stJudge3.getChildren().addAll((Object[])new Node[] { (Node)this.ciJudge3, (Node)txtJ3 });
      ...
      switch (this.hitEventType) {
        ...
        case TURNING_KICK_TECH:
          if (hitValue <= 100 || this.hitEventType
            ...
            this.stHit.getChildren().add(TkStrikeScoreboardGraphicDetailTypeUtil.getInstance().getNode4HeadImpact(scoreboardGraphicDetailType, controlHeight)); 
          break;
        case NONE:
        case null:
        case null:
        case null:
          if (hitValue <= 100 || this.hitEventType.equals(HitEventType.SPECIAL_BODY) || 
            isBackupSystemEnabled())
            this.stHit.getChildren().add(TkStrikeScoreboardGraphicDetailTypeUtil.getInstance().getNode4BodyImpact(scoreboardGraphicDetailType, controlHeight)); 
          break;
        case null:
          this.stHit.getChildren().add(TkStrikeScoreboardGraphicDetailTypeUtil.getInstance().getNode4PunchImpact(scoreboardGraphicDetailType, controlHeight));
          break;
      }  
    ...
    if (hitFirst) {
      getChildren().addAll((Object[])new Node[] { (Node)this.stHit, (Node)this.stJudge1, (Node)this.stJudge2, (Node)this.stJudge3 });
    } else {
      getChildren().addAll((Object[])new Node[] { (Node)this.stJudge1, (Node)this.stJudge2, (Node)this.stJudge3, (Node)this.stHit });
    } 
  }
  
  public TkStrikeCombatHit(TkStrikeScoreboardGraphicDetailType scoreboardGraphicDetailType, long hitTimestamp, HitEventType hitEventType, int hitValue, int judgesEnabled, int defaultFontSize, Double controlHeight, Double controlSpacing, Double controlJudgesWidth, Double controlHitWidth, boolean hitFirst, boolean backupSystemEnabled) {
    this(scoreboardGraphicDetailType, hitTimestamp, hitEventType, hitValue, judgesEnabled, defaultFontSize, controlHeight, controlSpacing, controlJudgesWidth, controlHitWidth, hitFirst, backupSystemEnabled, HitJudgeStatus.NOT_ENABLED, HitJudgeStatus.NOT_ENABLED, HitJudgeStatus.NOT_ENABLED);
  }
  ...
  public boolean isJudgesTechValidated() {
    switch (this.judgesEnabled) {
      case 1:
        return ((HitJudgeStatus)this.judge1HitStatus.getValue()).equals(HitJudgeStatus.VALIDATED);
      case 2:
        return (((HitJudgeStatus)this.judge1HitStatus.getValue()).equals(HitJudgeStatus.VALIDATED) && ((HitJudgeStatus)this.judge2HitStatus.getValue()).equals(HitJudgeStatus.VALIDATED));
      case 3:
        return ((((HitJudgeStatus)this.judge1HitStatus.getValue()).equals(HitJudgeStatus.VALIDATED) && ((HitJudgeStatus)this.judge2HitStatus.getValue()).equals(HitJudgeStatus.VALIDATED)) || (((HitJudgeStatus)this.judge1HitStatus
          .getValue()).equals(HitJudgeStatus.VALIDATED) && ((HitJudgeStatus)this.judge3HitStatus.getValue()).equals(HitJudgeStatus.VALIDATED)) || (((HitJudgeStatus)this.judge2HitStatus
          .getValue()).equals(HitJudgeStatus.VALIDATED) && ((HitJudgeStatus)this.judge3HitStatus.getValue()).equals(HitJudgeStatus.VALIDATED)));
    } 
    return false;
  }
  ...
  public HitJudgeStatus getJudge1Status() {
    return (HitJudgeStatus)this.judge1HitStatus.get();
  }
  
  public HitJudgeStatus getJudge2Status() {
    return (HitJudgeStatus)this.judge2HitStatus.get();
  }
  
  public HitJudgeStatus getJudge3Status() {
    return (HitJudgeStatus)this.judge3HitStatus.get();
  }
  ...
  private final class JudgeHitStatusChangeListener implements ChangeListener<HitJudgeStatus> {
    ...
    public void changed(ObservableValue<? extends HitJudgeStatus> observableValue, HitJudgeStatus hitJudgeStatus, HitJudgeStatus newJudgeStatus) {
          ...
          case null:
            switch (TkStrikeCombatHit.this.paraTkdTechEvent) {
              case SPINNING_KICK_TECH:
                ...
                polygon = new Polygon();
                polygon.getPoints().addAll((Object[])new Double[] { Double.valueOf(base * -1.0D), Double.valueOf(base), 
                      Double.valueOf(base), Double.valueOf(base), 
                      Double.valueOf(0.0D), Double.valueOf(base * -1.0D) });
                this.ciJudge = (Shape)polygon;
      ...
      this.ciJudge.setFill((Paint)fillColor);
    }
  }
}
