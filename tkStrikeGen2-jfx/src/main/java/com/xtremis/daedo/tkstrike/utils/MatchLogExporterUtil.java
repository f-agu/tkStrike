...
public class MatchLogExporterUtil implements InitializingBean {
  ...
  public void exportAggregatedsToCSVFile(String[][] defaultAggregatted, String[][] hitsByCategoryCount, File targetDirectory) throws TkStrikeServiceException {
    if (defaultAggregatted != null && hitsByCategoryCount != null && targetDirectory != null) {
      ...
      File defaultAggregatedFile = new File(targetDirectory.getAbsolutePath() + "/" + this.sdf4FileFullDate.format(current) + "-MatchLogAggregated.csv");
      exportToCSV(defaultAggregatted, defaultAggregatedFile);
      File hitsByCategoryCountFile = new File(targetDirectory.getAbsolutePath() + "/" + this.sdf4FileFullDate.format(current) + "-HitsByCategoryCount.csv");
      exportToCSV(hitsByCategoryCount, hitsByCategoryCountFile);
    } 
  }
  
  public void exportToCSVFile(CommonMatchLogDto matchLogEntry, List<? extends CommonMatchLogItemDto> matchLogItemEntries, File targetDirectory) throws TkStrikeServiceException {
    if (matchLogEntry != null && matchLogItemEntries != null && targetDirectory != null) {
      ...
      File fileMatchLog = new File(targetDirectory.getAbsolutePath() + "/" + this.sdf4FileFullDate.format(new Date(matchLogEntry.getMatchStartTime().longValue())) + "-" + matchLogEntry.getMatchNumber() + "-matchLog.csv");
      exportHeader(matchLogEntry, fileMatchLog);
      File fileMatchLogItems = new File(targetDirectory.getAbsolutePath() + "/" + this.sdf4FileFullDate.format(new Date(matchLogEntry.getMatchStartTime().longValue())) + "-" + matchLogEntry.getMatchNumber() + "-matchLogItems.csv");
      exportItems(matchLogItemEntries, fileMatchLogItems);
    } 
  }
  
  public void exportToXLSFile(CommonMatchLogDto matchLog, MatchInfo matchInfo, Map<Integer, DataTableRoundInfoItem> dataTablesItems, List<MatchHistoryItem> matchHistoryItems, File targetDirectory) throws TkStrikeServiceException {
    if (matchLog != null && dataTablesItems != null && targetDirectory != null) {
      ...
      File fileMatchLog = new File(targetDirectory.getAbsolutePath() + "/" + this.sdf4FileFullDate.format(new Date(matchLog.getMatchStartTime().longValue())) + "-" + matchLog.getMatchNumber() + "-matchLog.xls");
  ...
  public void exportToPDFFile(CommonMatchLogDto matchLogEntry, List<CommonMatchLogItemDto> matchLogItemEntries, File targetDirectory) throws TkStrikeServiceException {
    if (matchLogEntry != null && matchLogItemEntries != null && targetDirectory != null) {
      if (!targetDirectory.exists())
        try {
          FileUtils.forceMkdir(targetDirectory);
      ...
      File fileMatchLog = new File(targetDirectory.getAbsolutePath() + "/" + this.sdf4FileFullDate.format(new Date(matchLogEntry.getMatchStartTime().longValue())) + "-" + matchLogEntry.getMatchNumber() + "-matchLog.pdf");
      if (fileMatchLog.exists())
        FileUtils.deleteQuietly(fileMatchLog); 
      try {
  ...
  private byte[] generateMatchLogPDFReport(CommonMatchLogDto matchLogEntry, List<CommonMatchLogItemDto> matchLogItemEntries) throws TkStrikeServiceException {
  ...
  private void exportHeader(CommonMatchLogDto matchLogEntry, File targetFile) throws TkStrikeServiceException {
    ...
    try {
      ...
      csvWriter = new CSVWriter(new PrintWriter(fos), ',', false);
