...
public class NodeBatteryInfo extends StackPane {
  ...
  public NodeBatteryInfo(Integer groupNumber, String nodeId, SensorNodeType sensorNodeType, Boolean blue, boolean showPct, Double height, Double width, String textStyleClass) {
    ...
    this.rectangle.setStroke(SensorNodeType.JUDGE.equals(this.sensorNodeType) ? (Paint)Color.WHITE : (this.blue.booleanValue() ? (Paint)Color.BLUE : (Paint)Color.RED));
    this.rectangle.setFill((Paint)Color.RED);
    this.text = new Text("");
    ...
    if (StringUtils.isNotBlank(textStyleClass))
      this.text.getStyleClass().addAll((Object[])new String[] { textStyleClass }); 
    StackPane.setAlignment((Node)this.rectangle, Pos.CENTER);
    StackPane.setAlignment((Node)this.text, Pos.CENTER);
    getChildren().addAll((Object[])new Node[] { (Node)this.rectangle, (Node)this.text });
            ...
            public void run() {
              if (newValue.booleanValue()) {
                if (!NodeBatteryInfo.this.showPct) {
                  ...
                  try {
                    NodeBatteryInfo.this.text.setText((NodeBatteryInfo.this.battery != null) ? NodeBatteryInfo.pctFormat.format(NodeBatteryInfo.this.battery) : "");
                  } catch (Exception e) {
                    e.printStackTrace();
                  } 
                } 
                if (NodeBatteryInfo.this.offLine.booleanValue()) {
                  ...
                  NodeBatteryInfo.this.rectangle.setFill((Paint)Color.RED);
                } else if (NodeBatteryInfo.this.sensorOk.booleanValue() && NodeBatteryInfo.this.battery.doubleValue() > 20.0D) {
                  NodeBatteryInfo.this.rectangle.setFill((Paint)Color.GREEN);
                } else {
                  NodeBatteryInfo.this.rectangle.setFill((Paint)Color.ORANGE);
                } 
              } 
            }
          });
