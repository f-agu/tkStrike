...
public abstract class BaseTkStrikeMainController<MW extends CommonMatchWorker, MFRC extends CommonMatchFinalResultController, RFC extends CommonRoundFinishedConfirmationController>
		extends CommonTkStrikeBaseController
		implements TkStrikeMainController {
	...
	public final void openFinalResult() {
		openInNewStage(getMatchFinalResultController(),
	...
	public void addGamJeom2Blue() {
		if(_canAddGamJeom(true))
			getMatchWorker().addBlueGamJeom(ActionSource.MAIN_CONTROL, System.currentTimeMillis(), getMatchWorker()
	...
	private boolean _canAddGamJeom(boolean isBlue) {
		if(MatchVictoryCriteria.BESTOF3.equals(getMatchWorker().getMatchVictoryCriteria()) && MatchStatusId.ROUND_FINISHED
			...
			Alert alert = new Alert(Alert.AlertType.CONFIRMATION, getMessage("message.addGamJeom.confirmMessage"), new ButtonType[] {okPrevRound,
					okNextRound, cancel});
			...
			alert.showAndWait().ifPresent(response -> {
				if(response.getButtonData() == ButtonBar.ButtonData.BACK_PREVIOUS) {
					...
					if(isBlue) {
						...
						getMatchWorker().addRedGamJeom(ActionSource.MAIN_CONTROL, System.currentTimeMillis(), getMatchWorker()
								.getCurrentRoundCountdownAsMillis());
					}
				} else if(response.getButtonData() == ButtonBar.ButtonData.NEXT_FORWARD) {
					logger.info("Add to NextRound");
					if(isBlue) {
						getMatchWorker().addBlueGamJeomToNextRound(ActionSource.MAIN_CONTROL, System.currentTimeMillis(), getMatchWorker()
								.getCurrentRoundCountdownAsMillis());
					} else {
						getMatchWorker().addRedGamJeomToNextRound(ActionSource.MAIN_CONTROL, System.currentTimeMillis(), getMatchWorker()
								.getCurrentRoundCountdownAsMillis());
					}
	...
	private boolean _canAddPenalty() {
		return (getMatchWorker().getCurrentMatchStatus().equals(MatchStatusId.ROUND_WORKING) ||
				...
				getMatchWorker().getCurrentMatchStatus().equals(MatchStatusId.ROUND_KYESHI)) ? _addGamJeomConfirmation() : false;
	}
	...
	private boolean _canRemovePenalty() {
		return (getMatchWorker().getCurrentMatchStatus().equals(MatchStatusId.ROUND_WORKING) ||
				...
				getMatchWorker().getCurrentMatchStatus().equals(MatchStatusId.ROUND_FINISHED) ||
				getMatchWorker().getCurrentMatchStatus().equals(MatchStatusId.ROUND_PAUSED) ||
				getMatchWorker().getCurrentMatchStatus().equals(MatchStatusId.ROUND_KYESHI)) ? _removeGamJeomConfirmation() : false;
	}

	private boolean _addGamJeomConfirmation() {
		if(this.requireConfirmationForPenalty.booleanValue()) {
			Optional<ButtonType> opResponse = showConfirmDialog(getMessage("message.confirmDialog.title"), getMessage(
					"message.addGamJeom.confirmMessage"));
	...
	private boolean _removeGamJeomConfirmation() {
		if(this.requireConfirmationForPenalty.booleanValue()) {
			Optional opResponse = showConfirmDialog(getMessage("message.confirmDialog.title"), getMessage("message.removeGamJeom.confirmMessage"));
			return (opResponse != null && opResponse.isPresent() && opResponse.get().equals(ButtonType.OK));
		}
		return true;
	}
	...
	public final void showExternalScoreboard() {
		...
		esScene.getStylesheets().addAll(this.tkStrikeStyleSheetsHelper.getTkStrikeStyleSheets());
		...
		esStage.setFullScreenExitKeyCombination(new KeyCodeCombination(KeyCode.E, new KeyCombination.Modifier[] {KeyCombination.CONTROL_DOWN,
	...
	public final void hideExternalScoreboard() {
		if(getExternalScreenMainController().getRootView().getScene() != null) {
			...
			getExternalScreenMainController().getCurrentStage().fireEvent(new WindowEvent(getCurrentStage(), WindowEvent.WINDOW_CLOSE_REQUEST));
		}
	}
	...
	public final void afterPropertiesSet() throws Exception {
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
				if(newValue.booleanValue())
					Platform.runLater(new Runnable() {
						...
						@Override
						public void run() {
							BaseTkStrikeMainController.this.addStatusCSSToLabel(BaseTkStrikeMainController.this.lblStatusNetworkError,
									"appStatus-no");
							try {
								for(AppStatusId appStatusId : BaseTkStrikeMainController.this.getAppStatusWorker().getCurrentAppStatuses()) {
									if(AppStatusId.NETWORK_CONFIGURED.equals(appStatusId)) {
										BaseTkStrikeMainController.this.addStatusCSSToLabel(
												BaseTkStrikeMainController.this.lblStatusNetworkConfigured, "appStatus-ok");
									} else if(AppStatusId.MATCH_CONFIGURED.equals(appStatusId)) {
										BaseTkStrikeMainController.this.addStatusCSSToLabel(BaseTkStrikeMainController.this.lblStatusMatchConfigured,
												"appStatus-ok");
										...
										if(AppStatusId.READY_FOR_MATCH.equals(appStatusId)) {
											BaseTkStrikeMainController.this.addStatusCSSToLabel(BaseTkStrikeMainController.this.lblStatusReady4Match,
													"appStatus-ok");
		...
		getMatchWorker().currentMatchStatusProperty().addListener(new ChangeListener<MatchStatusId>() {
			...
			@Override
			public void changed(ObservableValue<? extends MatchStatusId> observableValue, MatchStatusId oldMatchStatusId,
								...
								switch(newMatchStatus) {
									...
									case null:
										...
										BaseTkStrikeMainController.this.btTimeout.setText(BaseTkStrikeMainController.this.getMessage(
												"btn.main.resume"));
										BaseTkStrikeMainController.this.btTimeout.setUserData(
												BaseTkStrikeMainController.TimeoutButtonUserData.RESUME);
										BaseTkStrikeMainController.this.btKyeShi.setText(BaseTkStrikeMainController.this.getMessage(
												"btn.main.kye-shi"));
										BaseTkStrikeMainController.this.btKyeShi.setUserData(BaseTkStrikeMainController.KyeShiButtonUserData.KYE_SHI);
										break;
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
				if(newValue.booleanValue())
					Platform.runLater(new Runnable() {
						...
						@Override
						public void run() {
							BaseTkStrikeMainController.this.openInNewStage(BaseTkStrikeMainController.this.getRoundFinishedController(),
									BaseTkStrikeMainController.this
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
				if(newValue.booleanValue())
					Platform.runLater(new Runnable() {
						...
						@Override
						public void run() {
							boolean accepted = BaseTkStrikeMainController.this.showMonologConfirmDialog(BaseTkStrikeMainController.this.getMessage(
									"message.confirmDialog.title"), BaseTkStrikeMainController.this.getMessage(
											"message.confirm.confirmPointGapByPenalties")).equals(MonologFXButton.Type.YES);
		...
		getAppStatusWorker().matchLogCSVGeneratedProperty().addListener(new ChangeListener<Boolean>() {
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
				if(newValue.booleanValue())
					BaseTkStrikeMainController.this.showInfoNotification(BaseTkStrikeMainController.this.getMessage(
							"message.info.matchLogCSVGenerated"));
			}
		});
		getAppStatusWorker().matchLogXLSGeneratedProperty().addListener(new ChangeListener<Boolean>() {
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
				if(newValue.booleanValue())
					BaseTkStrikeMainController.this.showInfoNotification(BaseTkStrikeMainController.this.getMessage(
							"message.info.matchLogXLSGenerated"));
			}
		});
		getAppStatusWorker().matchLogPDFGeneratedProperty().addListener(new ChangeListener<Boolean>() {
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
				if(newValue.booleanValue())
					BaseTkStrikeMainController.this.showInfoNotification(BaseTkStrikeMainController.this.getMessage(
							"message.info.matchLogPDFGenerated"));
			}
		});
		...
		getAppStatusWorker().rulesChanges().addListener(new ChangeListener<Boolean>() {
			...
			public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, final Boolean newValue) {
				Platform.runLater(new Runnable() {
					...
					public void run() {
						if(newValue.booleanValue()) {
							MatchVictoryCriteria byRules = BaseTkStrikeMainController.this.getAppStatusWorker().getRulesEntry()
									.getMatchVictoryCriteria();
							if(MatchVictoryCriteria.BESTOF3.equals(byRules)) {
								BaseTkStrikeMainController.this.lblMatchVictoryCriteria.setText(BaseTkStrikeMainController.this.getMessage(
										"toggle.matchVictoryCriteria.byRounds"));
							} else {
								BaseTkStrikeMainController.this.lblMatchVictoryCriteria.setText(BaseTkStrikeMainController.this.getMessage(
										"toggle.matchVictoryCriteria.byPoints"));
							}
						}
					}
				});
		...
		this.btTimeout.setOnAction(new EventHandler<ActionEvent>() {
			...
			@Override
			public void handle(ActionEvent actionEvent) {
				BaseTkStrikeMainController.TimeoutButtonUserData timeoutButtonUserData = (BaseTkStrikeMainController.TimeoutButtonUserData)BaseTkStrikeMainController.this.btTimeout
						.getUserData();
		...
		this.btKyeShi.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent actionEvent) {
				...
				BaseTkStrikeMainController.KyeShiButtonUserData validate = (BaseTkStrikeMainController.KyeShiButtonUserData)BaseTkStrikeMainController.this.btKyeShi
						.getUserData();
				switch(validate) {
					case KYE_SHI:
		...
		this.currentMatchStatus.addListener(new ControlEnableListener(this.pnButtonsPenalties, new MatchStatusId[] {MatchStatusId.ROUND_WORKING,
				MatchStatusId.WAITING_4_START_ROUND, MatchStatusId.WAITING_4_START_GOLDENPOINT, MatchStatusId.ROUND_IN_GOLDENPOINT,
				MatchStatusId.ROUND_KYESHI, MatchStatusId.ROUND_FINISHED, MatchStatusId.ROUND_PAUSED}));
		this.currentMatchStatus.addListener(new ControlEnableListener(this.pnButtonsMatchControl, new MatchStatusId[] {
				MatchStatusId.WAITING_4_MATCH, MatchStatusId.WAITING_4_START_ROUND, MatchStatusId.WAITING_4_START_GOLDENPOINT,
				MatchStatusId.ROUND_WORKING, MatchStatusId.ROUND_PAUSED, MatchStatusId.ROUND_KYESHI, MatchStatusId.ROUND_FINISHED,
				MatchStatusId.ROUND_PARA_TIMEOUT_WORKING, MatchStatusId.ROUND_IN_GOLDENPOINT, MatchStatusId.ROUND_GOLDENPOINT_FINISHED,
				MatchStatusId.MATCH_FINISHED, MatchStatusId.MATCH_NEEDS_CONFIRM_FINAL_DECISION, MatchStatusId.NETWORK_ERROR}));
		this.currentMatchStatus.addListener(new ControlEnableListener(this.btStartRound, new MatchStatusId[] {MatchStatusId.WAITING_4_MATCH,
				MatchStatusId.WAITING_4_START_ROUND, MatchStatusId.WAITING_4_START_GOLDENPOINT, MatchStatusId.ROUND_WORKING,
				MatchStatusId.ROUND_KYESHI, MatchStatusId.ROUND_PAUSED, MatchStatusId.ROUND_IN_GOLDENPOINT}));
		this.currentMatchStatus.addListener(new ControlEnableListener(this.btNextRound, new MatchStatusId[] {MatchStatusId.ROUND_PAUSED,
				MatchStatusId.ROUND_KYESHI, MatchStatusId.ROUND_FINISHED}));
		this.currentMatchStatus.addListener(new ControlEnableListener(this.btKyeShi, new MatchStatusId[] {MatchStatusId.WAITING_4_MATCH,
				MatchStatusId.WAITING_4_START_ROUND, MatchStatusId.WAITING_4_START_GOLDENPOINT, MatchStatusId.ROUND_WORKING,
				MatchStatusId.ROUND_IN_GOLDENPOINT, MatchStatusId.NETWORK_ERROR, MatchStatusId.ROUND_PAUSED, MatchStatusId.ROUND_KYESHI}));
		this.currentMatchStatus.addListener(new ControlEnableListener(this.btDoctor, new MatchStatusId[] {MatchStatusId.ROUND_KYESHI,
				MatchStatusId.WAITING_4_MATCH, MatchStatusId.WAITING_4_START_ROUND, MatchStatusId.WAITING_4_START_GOLDENPOINT,
				MatchStatusId.ROUND_WORKING, MatchStatusId.ROUND_IN_GOLDENPOINT, MatchStatusId.ROUND_PAUSED}));
		this.currentMatchStatus.addListener(new ControlEnableListener(this.btTimeout, new MatchStatusId[] {MatchStatusId.ROUND_WORKING,
				MatchStatusId.ROUND_IN_GOLDENPOINT, MatchStatusId.ROUND_PARA_TIMEOUT_WORKING, MatchStatusId.ROUND_PAUSED}));
		this.currentMatchStatus.addListener(new ControlEnableListener(this.btScoreboard, new MatchStatusId[] {MatchStatusId.WAITING_4_MATCH,
				MatchStatusId.ROUND_PAUSED, MatchStatusId.ROUND_PARA_TIMEOUT_WORKING, MatchStatusId.ROUND_KYESHI, MatchStatusId.ROUND_FINISHED,
				MatchStatusId.WAITING_4_START_ROUND, MatchStatusId.MATCH_NEEDS_CONFIRM_FINAL_DECISION, MatchStatusId.ROUND_GOLDENPOINT_FINISHED}));
		this.currentMatchStatus.addListener(new ControlEnableListener(this.btMatchLog, new MatchStatusId[] {MatchStatusId.ROUND_PAUSED,
				MatchStatusId.ROUND_PARA_TIMEOUT_WORKING, MatchStatusId.ROUND_KYESHI, MatchStatusId.ROUND_FINISHED,
				MatchStatusId.WAITING_4_START_ROUND, MatchStatusId.MATCH_FINISHED, MatchStatusId.MATCH_NEEDS_CONFIRM_FINAL_DECISION,
				MatchStatusId.ROUND_GOLDENPOINT_FINISHED}));
		this.currentMatchStatus.addListener(new ControlEnableListener(this.btFinalResult, new MatchStatusId[] {MatchStatusId.WAITING_4_MATCH,
				MatchStatusId.NETWORK_ERROR, MatchStatusId.ROUND_PARA_TIMEOUT_WORKING, MatchStatusId.ROUND_PAUSED, MatchStatusId.ROUND_KYESHI,
				MatchStatusId.ROUND_FINISHED, MatchStatusId.WAITING_4_START_ROUND, MatchStatusId.MATCH_NEEDS_CONFIRM_FINAL_DECISION,
				MatchStatusId.ROUND_GOLDENPOINT_FINISHED}));
		this.currentMatchStatus.addListener(new ControlEnableListener(this.btRingManager, new MatchStatusId[] {MatchStatusId.WAITING_4_MATCH,
				MatchStatusId.NETWORK_ERROR, MatchStatusId.NOT_READY, MatchStatusId.MATCH_FINISHED}));
		this.currentMatchStatus.addListener(new ControlEnableListener(this.btHardwareTest, new MatchStatusId[] {MatchStatusId.WAITING_4_MATCH,
				MatchStatusId.NETWORK_ERROR, MatchStatusId.ROUND_PAUSED, MatchStatusId.ROUND_FINISHED, MatchStatusId.WAITING_4_START_ROUND,
				MatchStatusId.NOT_READY, MatchStatusId.MATCH_FINISHED}));
		this.currentMatchStatus.addListener(new ControlEnableListener(this.btConfig, new MatchStatusId[] {MatchStatusId.WAITING_4_MATCH,
				MatchStatusId.NETWORK_ERROR, MatchStatusId.NOT_READY, MatchStatusId.MATCH_FINISHED}));
			...
			public void handle(KeyEvent event) {
				if(TkStrikeKeyCombinationsHelper.keyCombConfig.match(event) &&
					...
					BaseTkStrikeMainController.this.getMatchWorker().doChangeTogglesColorGroupSelectionVisible();
						...
						&& TkStrikeKeyCombinationsHelper.keyCombSelectBlueG1.match(event)) {
					if(BaseTkStrikeMainController.this.isConfirmedToChangeColorGroup(true, Integer.valueOf(1)))
						...
						&& TkStrikeKeyCombinationsHelper.keyCombSelectBlueG2.match(event)) {
					if(BaseTkStrikeMainController.this.isConfirmedToChangeColorGroup(true, Integer.valueOf(2)))
						...
						&& TkStrikeKeyCombinationsHelper.keyCombSelectBlueG3.match(event)) {
					if(BaseTkStrikeMainController.this.isConfirmedToChangeColorGroup(true, Integer.valueOf(3)))
						...
						&& TkStrikeKeyCombinationsHelper.keyCombSelectBlueG4.match(event)) {
					if(BaseTkStrikeMainController.this.isConfirmedToChangeColorGroup(true, Integer.valueOf(4)))
						...
						&& TkStrikeKeyCombinationsHelper.keyCombSelectBlueG5.match(event)) {
					if(BaseTkStrikeMainController.this.isConfirmedToChangeColorGroup(true, Integer.valueOf(5)))
						...
						&& TkStrikeKeyCombinationsHelper.keyCombSelectBlueG6.match(event)) {
					if(BaseTkStrikeMainController.this.isConfirmedToChangeColorGroup(true, Integer.valueOf(6)))
						...
						&& TkStrikeKeyCombinationsHelper.keyCombSelectRedG1.match(event)) {
					if(BaseTkStrikeMainController.this.isConfirmedToChangeColorGroup(false, Integer.valueOf(1)))
						...
						&& TkStrikeKeyCombinationsHelper.keyCombSelectRedG2.match(event)) {
					if(BaseTkStrikeMainController.this.isConfirmedToChangeColorGroup(false, Integer.valueOf(2)))
						...
						&& TkStrikeKeyCombinationsHelper.keyCombSelectRedG3.match(event)) {
					if(BaseTkStrikeMainController.this.isConfirmedToChangeColorGroup(false, Integer.valueOf(3)))
						...
						&& TkStrikeKeyCombinationsHelper.keyCombSelectRedG4.match(event)) {
					if(BaseTkStrikeMainController.this.isConfirmedToChangeColorGroup(false, Integer.valueOf(4)))
						...
						&& TkStrikeKeyCombinationsHelper.keyCombSelectRedG5.match(event)) {
					if(BaseTkStrikeMainController.this.isConfirmedToChangeColorGroup(false, Integer.valueOf(5)))
						...
						&& TkStrikeKeyCombinationsHelper.keyCombSelectRedG6.match(event) &&
				...
				if( ! BaseTkStrikeMainController.this.getMatchWorker().getCurrentMatchStatus().equals(MatchStatusId.ROUND_WORKING) &&
					...
					if(TkStrikeKeyCombinationsHelper.keyCombP1.match(event) || TkStrikeKeyCombinationsHelper.keyCombNumP1.match(event)) {
						...
						BaseTkStrikeMainController.this.getMatchWorker().addBlueBodyPoint(rulesEntry.getBodyPoints() * - 1,
								ActionSource.SCOREBOARD_EDITOR,

								System.currentTimeMillis(), currentRoundCountdown, 0);
					} else if(TkStrikeKeyCombinationsHelper.keyCombP2.match(event) || TkStrikeKeyCombinationsHelper.keyCombNumP2.match(event)) {
						BaseTkStrikeMainController.this.getMatchWorker().addBlueBodyTechPoint(rulesEntry.getBodyTechPoints(),
								ActionSource.SCOREBOARD_EDITOR,
						...
						BaseTkStrikeMainController.this.getMatchWorker().addBlueBodyTechPoint(rulesEntry.getBodyTechPoints() * - 1,
								ActionSource.SCOREBOARD_EDITOR,
						...
						BaseTkStrikeMainController.this.getMatchWorker().addBlueHeadPoint(rulesEntry.getHeadPoints() * - 1,
								ActionSource.SCOREBOARD_EDITOR,

								System.currentTimeMillis(), currentRoundCountdown, 0);
					} else if(TkStrikeKeyCombinationsHelper.keyCombP4.match(event) || TkStrikeKeyCombinationsHelper.keyCombNumP4.match(event)) {
						BaseTkStrikeMainController.this.getMatchWorker().addBlueHeadTechPoint(rulesEntry.getHeadTechPoints(),
								ActionSource.SCOREBOARD_EDITOR,

								System.currentTimeMillis(), currentRoundCountdown);
					} else if(TkStrikeKeyCombinationsHelper.keyCombM4.match(event) || TkStrikeKeyCombinationsHelper.keyCombNumM4.match(event)) {
						BaseTkStrikeMainController.this.getMatchWorker().addBlueHeadTechPoint(rulesEntry.getHeadTechPoints() * - 1,
								ActionSource.SCOREBOARD_EDITOR,

								System.currentTimeMillis(), currentRoundCountdown);
					} else if(TkStrikeKeyCombinationsHelper.keyCombP5.match(event) || TkStrikeKeyCombinationsHelper.keyCombNumP5.match(event)) {
						BaseTkStrikeMainController.this.getMatchWorker().addBluePunchPoint(rulesEntry.getPunchPoints(),
								ActionSource.SCOREBOARD_EDITOR,

								System.currentTimeMillis(), currentRoundCountdown);
					} else if(TkStrikeKeyCombinationsHelper.keyCombM5.match(event) || TkStrikeKeyCombinationsHelper.keyCombNumM5.match(event)) {
						BaseTkStrikeMainController.this.getMatchWorker().addBluePunchPoint(rulesEntry.getPunchPoints() * - 1,
								ActionSource.SCOREBOARD_EDITOR,
						...
						BaseTkStrikeMainController.this.getMatchWorker().addRedBodyPoint(rulesEntry.getBodyPoints() * - 1,
								ActionSource.SCOREBOARD_EDITOR,

								System.currentTimeMillis(), currentRoundCountdown, 0);
					} else if(TkStrikeKeyCombinationsHelper.keyCombP7.match(event) || TkStrikeKeyCombinationsHelper.keyCombNumP7.match(event)) {
						BaseTkStrikeMainController.this.getMatchWorker().addRedBodyTechPoint(rulesEntry.getBodyTechPoints(),
								ActionSource.SCOREBOARD_EDITOR,

								System.currentTimeMillis(), currentRoundCountdown);
					} else if(TkStrikeKeyCombinationsHelper.keyCombM7.match(event) || TkStrikeKeyCombinationsHelper.keyCombNumM7.match(event)) {
						BaseTkStrikeMainController.this.getMatchWorker().addRedBodyTechPoint(rulesEntry.getBodyTechPoints() * - 1,
								ActionSource.SCOREBOARD_EDITOR,
						...
						BaseTkStrikeMainController.this.getMatchWorker().addRedHeadPoint(rulesEntry.getHeadPoints() * - 1,
								ActionSource.SCOREBOARD_EDITOR,

								System.currentTimeMillis(), currentRoundCountdown, 0);
					} else if(TkStrikeKeyCombinationsHelper.keyCombP9.match(event) || TkStrikeKeyCombinationsHelper.keyCombNumP9.match(event)) {
						BaseTkStrikeMainController.this.getMatchWorker().addRedHeadTechPoint(rulesEntry.getHeadTechPoints(),
								ActionSource.SCOREBOARD_EDITOR,

								System.currentTimeMillis(), currentRoundCountdown);
					} else if(TkStrikeKeyCombinationsHelper.keyCombM9.match(event) || TkStrikeKeyCombinationsHelper.keyCombNumM9.match(event)) {
						BaseTkStrikeMainController.this.getMatchWorker().addRedHeadTechPoint(rulesEntry.getHeadTechPoints() * - 1,
								ActionSource.SCOREBOARD_EDITOR,
						...
						BaseTkStrikeMainController.this.getMatchWorker().addRedPunchPoint(rulesEntry.getPunchPoints() * - 1,
								ActionSource.SCOREBOARD_EDITOR,
		...
		this.tgBackupSystem.setOnAction(new EventHandler<ActionEvent>() {
			...
			@Override
			public void handle(ActionEvent event) {
				if( ! BaseTkStrikeMainController.this.showMonologConfirmDialog(BaseTkStrikeMainController.this.getMessage("title.default.question"),
						BaseTkStrikeMainController.this.getMessage("message.confirm.enableBackupSystem."
								+ BaseTkStrikeMainController.this.tgBackupSystem.isSelected())).equals(MonologFXButton.Type.YES)) {
					BaseTkStrikeMainController.this.tgBackupSystem.setSelected( ! BaseTkStrikeMainController.this.tgBackupSystem.isSelected());
				} else {
					BaseTkStrikeMainController.this.getMatchWorker().backupSystemEnabled().setValue(Boolean.valueOf(
							BaseTkStrikeMainController.this.tgBackupSystem.isSelected()));
				}
			}
		});
		...
		this.tgExternalScoreboard.selectedProperty().addListener(new ChangeListener<Boolean>() {
			...
			public void changed(ObservableValue<? extends Boolean> observableValue, Boolean aBoolean, Boolean t1) {
				if(t1.booleanValue()) {
					...
					BaseTkStrikeMainController.this.tgExternalScoreboard.setText(BaseTkStrikeMainController.this.getMessage(
							"label.externalScoreboard.on"));
				} else {
					BaseTkStrikeMainController.this.hideExternalScoreboard();
					BaseTkStrikeMainController.this.tgExternalScoreboard.setText(BaseTkStrikeMainController.this.getMessage(
							"label.externalScoreboard.off"));
				}
			}
		});
	}
	...
	private void addStatusCSSToLabel(Label label, String css2Add) {
		label.getStyleClass().removeAll(new String[] {"appStatus-no", "appStatus-warn", "appStatus-error", "appStatus-ok"});
		label.getStyleClass().addAll(new String[] {css2Add});
	}
