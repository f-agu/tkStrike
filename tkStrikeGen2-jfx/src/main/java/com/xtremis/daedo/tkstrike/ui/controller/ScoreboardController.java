  ...
  protected void _internalAfterPropertiesSet() throws Exception {
    ...
    this.ivLeftRoundsWinsVideo.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>() {
          public void handle(MouseEvent event) {
            if (event.getButton().equals(MouseButton.PRIMARY) && ScoreboardController.this
              .matchWorker.getCurrentMatchStatus().equals(MatchStatusId.ROUND_FINISHED))
    ...
    this.txtBodyLevel.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>() {
          public void handle(MouseEvent mouseEvent) {
            if (mouseEvent.getClickCount() == 2 && !ScoreboardController.this.matchWorker.getCurrentMatchStatus().equals(MatchStatusId.ROUND_WORKING) && 
          ...
          public void handle(KeyEvent keyEvent) {
            if (keyEvent.getCode().equals(KeyCode.TAB) || keyEvent.getCode().equals(KeyCode.ENTER)) {
              if (!ScoreboardController.this.txtChangeBodyLevel.getText().equals(ScoreboardController.this.txtBodyLevel.getText())) {
                if (ScoreboardController.this.showMonologConfirmDialog(ScoreboardController.this.getMessage("title.default.question"), ScoreboardController.this
                    .getMessage("message.confirmChangeBodyLevel", new String[] { ScoreboardController.access$200(this.this$0).getText() })).equals(MonologFXButton.Type.YES))
                  ScoreboardController.this.matchWorker.setMinBodyLevel(Integer.parseInt(ScoreboardController.this.txtChangeBodyLevel.getText())); 
          ...
          public void handle(KeyEvent keyEvent) {
            if (keyEvent.getCode().equals(KeyCode.ENTER) || keyEvent.getCode().equals(KeyCode.TAB)) {
              if (!ScoreboardController.this.txtChangeHeadLevel.getText().equals(ScoreboardController.this.txtHeadLevel.getText())) {
                if (ScoreboardController.this.showMonologConfirmDialog(ScoreboardController.this.getMessage("title.default.question"), ScoreboardController.this.getMessage("message.confirmChangeHeadLevel", new String[] { ScoreboardController.access$300(this.this$0).getText() })).equals(MonologFXButton.Type.YES))
                  ScoreboardController.this.matchWorker.setMinHeadLevel(Integer.parseInt(ScoreboardController.this.txtChangeHeadLevel.getText())); 
                ScoreboardController.this.getAppStatusWorker().doDialogWindowCloses();
              } 
              ScoreboardController.this.getRootView().requestFocus();
            } 
  ...
  class VideoQuotaClickEventHandler implements EventHandler<MouseEvent> {
    ...
    public void handle(final MouseEvent mouseEvent) {
      if (mouseEvent.getClickCount() >= 1) {
        ...
        if (mouseEvent.getButton().equals(MouseButton.PRIMARY) && ScoreboardController.this
          .matchWorker.getCurrentMatchStatus().equals(MatchStatusId.ROUND_PAUSED)) {
          ScoreboardController.this.doCallVideoRequest(this.isBlue);
        } else if (mouseEvent.getButton().equals(MouseButton.SECONDARY) && (ScoreboardController.this
          .matchWorker.getCurrentMatchStatus().equals(MatchStatusId.ROUND_PAUSED) || ScoreboardController.this
          .matchWorker.getCurrentMatchStatus().equals(MatchStatusId.WAITING_4_MATCH) || ScoreboardController.this
          .matchWorker.getCurrentMatchStatus().equals(MatchStatusId.WAITING_4_START_GOLDENPOINT) || ScoreboardController.this
          .matchWorker.getCurrentMatchStatus().equals(MatchStatusId.WAITING_4_START_ROUND) || ScoreboardController.this
          .matchWorker.getCurrentMatchStatus().equals(MatchStatusId.ROUND_FINISHED) || ScoreboardController.this
          .matchWorker.getCurrentMatchStatus().equals(MatchStatusId.MATCH_FINISHED) || ScoreboardController.this
          .matchWorker.getCurrentMatchStatus().equals(MatchStatusId.NETWORK_ERROR) || ScoreboardController.this
          .matchWorker.getCurrentMatchStatus().equals(MatchStatusId.ROUND_KYESHI))) {
          ...
          if (!mrContextMenu.isShowing())
            Platform.runLater(new Runnable() {
                  public void run() {
                    mrContextMenu.show((Node)ScoreboardController.VideoQuotaClickEventHandler.this.pnVideo, mouseEvent.getScreenX(), mouseEvent.getScreenY());
                  }
                }); 
        } 
      } 
