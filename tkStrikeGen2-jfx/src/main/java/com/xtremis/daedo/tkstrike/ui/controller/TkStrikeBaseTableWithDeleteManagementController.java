...
public abstract class TkStrikeBaseTableWithDeleteManagementController<E extends Entity, D extends Entry, S extends TkStrikeService<E, D>> extends TkStrikeBaseTableManagementController<E, D, S> {
  protected class DeleteEntryCell extends TableCell<D, Boolean> {
    final Button cellButton = new Button();
    
    public DeleteEntryCell(final TableView<D> tableView) {
      this.cellButton.getStyleClass().addAll((Object[])new String[] { "button-image-delete" });
      this.cellButton.setOnAction(new EventHandler<ActionEvent>() {
            public void handle(ActionEvent t) {
              ...
              Entry entry = (Entry)tableView.getItems().get(selectedIndex);
                ...
                if (TkStrikeBaseTableWithDeleteManagementController.this.canDeleteEntryById(entry.getId())) {
                  ...
                  if (MonologFXButton.Type.YES.equals(type)) {
                    try {
                      ...
                      TkStrikeBaseTableWithDeleteManagementController.this.manageException((Throwable)e, "Deleting entry", null);
                    } 
                    TkStrikeBaseTableWithDeleteManagementController.this.refreshTable();
                  } 
                } else {
                  TkStrikeBaseTableWithDeleteManagementController.this.showErrorDialog(TkStrikeBaseTableWithDeleteManagementController.this.getMessage("title.default.error"), TkStrikeBaseTableWithDeleteManagementController.this.getMessageEntityCantBeDeleted());
                }  
            }
          });
    }
